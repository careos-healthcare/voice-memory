...
class ConfirmedRepeatHelpfulActionPayoffCard extends StatelessWidget {
  ...
  final ConfirmedRepeatHelpfulActionPayoff payoff;
  ...
  Widget build(BuildContext context) {
    ...
    if (surface != null && count != null) {
      EarlyArchiveProofAnalytics.helpfulActionPayoffSeen(
        entryCount: count,
        surface: surface,
      );
    }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            payoff.title,
            key: const Key('confirmed_repeat_helpful_action_payoff_title'),
            style: titleStyle,
          ),
          const SizedBox(height: AppSpacing.sm),
          Text(
            payoff.body,
            key: const Key('confirmed_repeat_helpful_action_payoff_body'),
            style: bodyStyle,
          ),
          const SizedBox(height: AppSpacing.sm),
          for (final line in payoff.evidenceLines)
            Padding(
              ...
              child: Text(
                line,
                key: ValueKey(
                  'confirmed_repeat_helpful_action_payoff_evidence_$line',
                ),
                style: evidenceStyle,
              ),
            ),
          ...
          OutlinedButton(
            key: const Key(
              'confirmed_repeat_helpful_action_payoff_view_evidence_cta',
            ),
            onPressed: () {
              if (surface != null && count != null) {
          ...
          if (surface != null && count != null) ...[
            EarlyArchiveInsightWhySection(
              reasons: whyReasons,
              insightKey: 'helpfulActionPayoff',
            ),

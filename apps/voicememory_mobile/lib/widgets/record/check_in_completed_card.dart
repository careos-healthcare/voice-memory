...
import '../../features/tomorrow_return/check_in_result_interpretation.dart';
...
import '../trial/check_in_result_rating_prompt.dart';
import 'make_result_more_useful_sheet.dart';
...
class _CheckInCompletedCardState extends State<CheckInCompletedCard> {
  ...
  void initState() {
    ...
    ActivationTracker.trackActivationUsefulTakeawayShown();
    if (_showWhy) {
  ...
  Future<void> _onMakeMoreUseful() async {
    ActivationTracker.trackActivationMakeUsefulTapped();
    final reason = await MakeResultMoreUsefulSheet.show(context);
    if (reason == null || !mounted) return;
    ActivationTracker.trackActivationMakeUsefulReasonSelected();

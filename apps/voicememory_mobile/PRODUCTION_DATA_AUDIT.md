...
| File | Purpose | User-visible? | Safe for production? |
|------|---------|---------------|----------------------|
| `lib/features/belief_evolution/belief_evolution_service.dart` | `_seedFirstBelief` = first **version record** from user reflections | Yes when ≥ min evidence | Yes — gated by `archiveHasMinimumEvidence` |

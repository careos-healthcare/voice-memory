...
void main() {
  ...
  group('Privacy explanation copy', () {
    ...
    testWidgets('privacy explanation sheet renders ArchiveMe copy', (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          theme: AppTheme.light(),
          home: Builder(
  ...
  group('Clear local archive', () {
    testWidgets('requires confirmation dialog with cancel and confirm actions', (
      tester,
    ) async {
      await tester.pumpWidget(
        MaterialApp(
      ...
      expect(find.text(PrivacyDataControlsCopy.clearLocalArchiveConfirmBody), findsOneWidget);
    ...
    test('confirm clears local journal entries and archive-derived state', () async {
      await _seedJournal(
        journal,
        _entry(
          id: 'e1',
      ...
      ArchiveInsightFeedbackStore.record(
        ArchiveInsightFeedbackStore.targetId(ArchiveInsightTarget.beliefEvidence),
      ...
      await prefs.writeMap('pinnedEvidence', {'ids': ['e1']});
    ...
    test('share-safe proof does not include old private entry text after clear', () async {
      const sensitive = 'Maria told me about the divorce paperwork at the hospital again';

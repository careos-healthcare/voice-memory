...
void main() {
  ...
  group('ArchiveInsightFeedbackStore correction notes', () {
    test('empty correction note is not saved', () {
      const id = 'archive_home_three';
      expect(
        ArchiveInsightFeedbackStore.saveCorrectionNote(id, '   '),
        isFalse,
      );
      expect(ArchiveInsightFeedbackStore.hasCorrectionNote(id), isFalse);
    });

    test('valid correction note is stored locally for that target', () {
      const id = 'archive_home_three';
      expect(
    ...
    test('correction note does not affect unrelated targets', () {
    ...
    test('correction note is capped safely at 240 characters', () {
      const id = 'weeklyReview';
      final longNote = 'a' * 300;
      expect(
        ArchiveInsightFeedbackStore.saveCorrectionNote(id, longNote),
        isTrue,
      );
      expect(
    ...
    test('correction note can be replaced with latest note only', () {
      const id = 'beliefUpdate';
      ArchiveInsightFeedbackStore.saveCorrectionNote(id, 'First note');
      ArchiveInsightFeedbackStore.saveCorrectionNote(id, 'Updated note');
  ...
  group('Correction note privacy', () {
    test('shareable archive proof does not include correction notes', () {
      ArchiveInsightFeedbackStore.saveCorrectionNote(

...
void main() {
  ...
  group('FirstRunPositioningEngine', () {
    test('visible at zero entries if priority allows', () {
      final result = _result(entryCount: 0);
      expect(_shouldShow(result: result, entryCount: 0), isTrue);
    ...
    test('hidden recording/degraded', () {
      final result = _result(entryCount: 0);
      expect(
        _shouldShow(result: result, entryCount: 0, isRecording: true),
        isFalse,
      );
      expect(
        _shouldShow(
    ...
    test('metadata-only analytics', () {
      FirstRunPositioningAnalytics.seen(result: _result(entryCount: 0));

...
void main() {
  ...
  test('same result increments showedAgainCount and becomes active', () {
    var memory = _engine.apply(null, _update(PatternMemoryResultHint.same, day: 1),
        patternTitle: _title);
    memory = _engine.apply(
        memory, _update(PatternMemoryResultHint.same, day: 2),
        patternTitle: _title);
  ...
  test('two lighter results set easing', () {
    var memory = _engine.apply(
        null, _update(PatternMemoryResultHint.lighter, day: 1),
        patternTitle: _title);
    memory = _engine.apply(
        memory, _update(PatternMemoryResultHint.lighter, day: 2),
        patternTitle: _title);
  ...
  test('two heavier results set needsAttention', () {
    var memory = _engine.apply(
        null, _update(PatternMemoryResultHint.heavier, day: 1),
        patternTitle: _title);
    memory = _engine.apply(
        memory, _update(PatternMemoryResultHint.heavier, day: 2),
        patternTitle: _title);
  ...
  test('two changed results set changing', () {
    var memory = _engine.apply(
        null, _update(PatternMemoryResultHint.changed, day: 1),
        patternTitle: _title);
    memory = _engine.apply(
        memory, _update(PatternMemoryResultHint.changed, day: 2),
        patternTitle: _title);
  ...
  test('helped moments extracted only for lighter results', () {
    final lighter = _engine.apply(
      null,
      _update(PatternMemoryResultHint.lighter,
    ...
    final heavier = _engine.apply(
      null,
      _update(PatternMemoryResultHint.heavier,
          text: 'I paused before answering and it eased.'),
      patternTitle: _title,
    );
  ...
  test('three lighter check-ins feed a gettingLighter progress moment', () {
    ...
    memory = _engine.apply(
        ...
        patternTitle: _title);
    memory = _engine.apply(
        memory, _update(PatternMemoryResultHint.lighter, day: 3),
        patternTitle: _title);

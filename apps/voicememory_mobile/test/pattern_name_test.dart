...
import 'package:voicememory_mobile/features/pattern_naming/pattern_name_analytics.dart';
...
void main() {
  ...
  group('PatternNameStore', () {
    test('tapping Yes dismisses for that pattern', () {
      final entries = _threeSaidYesEntries();
      final prompt = PatternNameEngine.buildPrompt(entries: entries);
      expect(prompt, isNotNull);

      PatternNameStore.confirm(prompt!.patternKey);
      expect(PatternNameStore.isResolved(prompt.patternKey), isTrue);
      expect(PatternNameEngine.buildPrompt(entries: entries), isNull);
    });

    test('saving rename updates display label', () {
      const key = 'said yes';
      PatternNameStore.setCustomName(key, 'Agreeing when tired');
      expect(
    ...
    test('evidence phrase remains unchanged in belief surface', () {
      final entries = _threeSaidYesEntries();
      ...
      PatternNameStore.setCustomName(
    ...
    test('reset archive clears custom name', () async {
      PatternNameStore.setCustomName('said yes', 'Custom name');
  ...
  group('integration untouched', () {
    ...
    test('weekly review still works with renamed display label', () {
      ...
      final relabeled = weeklyReviewSurface.WeeklyArchiveReviewEngine.build(
        entries: entries,
        viewingConfirmedRepeatOrTimeline: true,
      );

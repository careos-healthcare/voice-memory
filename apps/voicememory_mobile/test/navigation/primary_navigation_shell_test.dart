...
import 'package:voicememory_mobile/router/primary_navigation_controller.dart';
import 'package:voicememory_mobile/router/record_navigation_activity_controller.dart';
...
void main() {
  ...
  group('primary destination model', () {
    test('has exactly four ordered routes and excludes graph', () {
      expect(PrimaryDestination.values, [
  ...
  group('four-branch shell behavior', () {
    testWidgets('defaults to Record and exposes four destinations in order', (
      tester,
    ) async {
      ...
      expect(_selectedPhoneIndex(tester), PrimaryDestination.record.index);
    ...
    testWidgets('each destination renders a real branch without a modal', (
      ...
      for (final destination in PrimaryDestination.values.skip(1)) {
        await _tapDestination(tester, destination.label);
        expect(find.text('${destination.label} branch'), findsOneWidget);
        expect(_selectedPhoneIndex(tester), destination.index);
        expect(
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await _tapDestination(tester, 'Changes');
        await _tapDestination(tester, 'Archive');
    ...
    testWidgets('a real detail pops before switching to Record', (
      ...
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('archive_root')), findsOneWidget);
      expect(_selectedPhoneIndex(tester), PrimaryDestination.archive.index);
    });
  ...
  group('responsive and accessible navigation', () {
    ...
    testWidgets('tablet uses a NavigationRail with four destinations', (
      tester,
    ) async {
  ...
  group('architecture guards', () {
    ...
    test('production does not directly push a primary destination', () {
      ...
      final forbidden = RegExp(
        r'''context\.push(?:<[^>]+>)?\(\s*(?:RouteCatalog\.(?:recordHome|archiveHome|changesHome|accountHome)|['"]/(?:record|archive-belief|belief-changes|account)['"])''',
      );
            ...
            StatefulShellBranch(
              ...
              routes: [
                GoRoute(
                  ...
                  builder: (_, _) => const _BranchPage(label: 'Changes'),
                ),
              ],
            ),
            StatefulShellBranch(

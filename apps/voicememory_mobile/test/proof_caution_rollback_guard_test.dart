...
void main() {
  ...
  group('ProofCautionGuardEngine upgrades', () {
    ...
    test('emerging with 4 entries + change anchor + recent return upgrades', () {
      ...
      expect(result.calibration.primaryCopy, ProofCautionGuardCopy.upgradeBody);
      expect(result.adjustedLevel, isNot(ProofConfidenceLevel.strong));
    });
  ...
  group('ProofCautionGuardEngine blocks', () {
    ...
    test('no safe anchor blocks upgrade', () {
      ...
      expect(
        result.blockedReason,
        ProofCautionGuardBlockedReason.noSafeAnchor,
      );
    });
  ...
  group('ProofCautionGuardAnalytics', () {
    test('metadata-only analytics', () {
      ...
      final applied = analyticsEvents
          ...
          .props;
      expect(applied.keys, containsAll(['source', 'original_level', 'adjusted_level', 'reason']));
  ...
  group('Protected areas', () {
    test('no threshold constant changes in pattern match quality engine', () {
      final source =
          File('lib/features/pattern_match_quality/pattern_match_quality_engine.dart')
              .readAsStringSync();
      expect(source, contains('static const minEntryCount = 2'));
      expect(source, contains('if (score >= 75) return PatternMatchConfidenceBand.strong'));
      expect(source, contains('if (score >= 55) return PatternMatchConfidenceBand.solid'));
    });
    ...
    test('no medical claims in guard copy', () {
      ...
      expect(ProofCautionGuardCopy.passesMedicalGuard(ProofCautionGuardCopy.upgradeBody), isTrue);
    });

    test('calibration engine wires guard without changing resolve thresholds', () {
      final source = File(

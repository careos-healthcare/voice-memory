...
void main() {
  group('PostSaveComparisonController', () {
    test('processMomentComparison runs model pipeline into success state', () async {
    ...
    test('processMomentComparison falls back to local engine when remote fails',
        () async {
      final controller = PostSaveComparisonController(
      ...
      expect(success.viewState.whatChangedText, startsWith('Evaluated locally.'));
    });

    test('processMomentComparison falls back when remote returns empty payload',
        () async {
      final controller = PostSaveComparisonController(
    ...
    test('processMomentComparison falls back when remote response is unparseable',
        () async {
      final controller = PostSaveComparisonController(
    ...
    test('processMomentComparison surfaces failure when remote and local fail',
        () async {
      final controller = PostSaveComparisonController(
        apiClient: _FakeModelApiClient(error: Exception('network down')),
        prefs: _FakePreferenceStore(),

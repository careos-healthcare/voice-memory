...
import 'package:voicememory_mobile/features/proof_admission/proof_admission_cache.dart';
...
final _createdAt = DateTime.utc(2026, 7, 1);
...
void main() {
  ...
  group('bounded size and eviction', () {
    ...
    test('the least recently used revision is the one evicted', () {
      ...
      cache.revisionFor(_entry(entryId: 'entry-1'));
  ...
  group('cache keys', () {
    test('no key contains a transcript, a quote or a statement', () {
      final cache = ProofAdmissionCache();
      cache
        ..revisionFor(_entry(transcript: _transcript))
        ..sourceEntryFor(
          entryId: 'entry-2',
          archiveScope: 'archive-a',
          ownerScope: 'owner-1',

...
void main() {
  ...
  group('YesterdaysSnapshotScreen', () {
    testWidgets('shows hook prompt and three micro-review bullets', (tester) async {
      String? handoffRoute;
      ...
      expect(find.byKey(const Key('yesterdays_snapshot_prompt')), findsOneWidget);
      expect(find.text(_hook().dynamicPrompt), findsOneWidget);
      expect(find.byKey(const Key('yesterdays_snapshot_micro_review')), findsOneWidget);
      expect(find.byKey(const Key('yesterdays_snapshot_bullet_0')), findsOneWidget);
      expect(find.byKey(const Key('yesterdays_snapshot_bullet_1')), findsOneWidget);
      expect(find.byKey(const Key('yesterdays_snapshot_bullet_2')), findsOneWidget);
      expect(find.byKey(const Key('yesterdays_snapshot_reaction_bar')), findsOneWidget);
      expect(handoffRoute, isNull);
    });

    testWidgets('reaction tap transitions to recording and triggers handoff', (tester) async {
      String? handoffRoute;
      ...
      await tester.tap(
        find.byKey(
          Key('yesterdays_snapshot_reaction_${YesterdaysSnapshotReaction.stuck.name}'),
        ),
      );
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 300));

      expect(find.byKey(const Key('yesterdays_snapshot_recording')), findsOneWidget);
      expect(find.byKey(const Key('yesterdays_snapshot_record_cta')), findsOneWidget);
      ...
      final reactionEvents = telemetry.events
          .where((e) => e.event == CuriosityTelemetryTracker.reactionTappedEvent)
          .toList();
      expect(reactionEvents, hasLength(1));
      expect(reactionEvents.single.meta['hook_id'], 'hook_1');
      expect(reactionEvents.single.meta['reaction'], YesterdaysSnapshotReaction.stuck.name);

      expect(reactionRepository.records, hasLength(1));
      expect(reactionRepository.records.single.hookId, 'hook_1');
      expect(reactionRepository.records.single.reactionType, YesterdaysSnapshotReaction.stuck);
      expect(reactionRepository.records.single.primaryAnchor, 'said yes again');
      expect(reactionRepository.records.single.hookType, CuriosityHookType.blocker);
    });

    testWidgets('shows synthesized prompt instead of stored dynamicPrompt', (tester) async {

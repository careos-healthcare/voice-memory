...
void main() {
  group('CuriosityHookCard Adaptive Layout Tests', () {
    ...
    testWidgets(
        'renders breath gate when low cognitive load is active',
        (WidgetTester tester) async {
      ...
      expect(find.byKey(const Key('curiosity_hook_response_input')), findsNothing);
  ...
  group('CuriosityHookCard Interactive State Machine Validation', () {
    testWidgets(
      ...
      expect(
        ...
        isNull,
        reason:
            'Action button must remain unclickable during active pacing!',
      );
    });
    ...
    testWidgets(
      ...
      await tester.enterText(find.byType(TextField), 'Calm structured recovery.');
  ...
  group('ConnectedCuriosityHookCard', () {
    testWidgets('loads baseline metrics and renders grounded layout on overload',
        (tester) async {

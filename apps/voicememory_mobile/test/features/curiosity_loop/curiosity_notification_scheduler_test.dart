...
import 'package:voicememory_mobile/features/curiosity_loop/yesterdays_snapshot_copy.dart';
...
void main() {
  ...
  group('CuriosityNotificationPayload', () {
    ...
    test('decode ignores unrelated payloads', () {
      ...
      expect(CuriosityNotificationPayload.decodeHookId('check_in:tci1'), isNull);
      expect(
  ...
  group('CuriosityNotificationScheduler', () {
    test('scheduleCuriosityNotification returns false when unavailable', () async {
    ...
    test('scheduleCuriosityNotification returns false when fire time passed', () async {
      final scheduler = CuriosityNotificationScheduler();
      final hook = _hook(
        id: 'hook_past',
    ...
    test('initialize and requestPermissions do not throw without plugin', () async {
      final scheduler = CuriosityNotificationScheduler();
    ...
    test('notification id is stable for hook id', () {
      expect(
        CuriosityNotificationScheduler.instance().isAvailable,
        isFalse,
      );
      final payload = CuriosityNotificationPayload.encode('curiosity_stable');
      expect(CuriosityNotificationPayload.decodeHookId(payload), 'curiosity_stable');
    });
  });
}

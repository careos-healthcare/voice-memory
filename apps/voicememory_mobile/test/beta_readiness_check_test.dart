...
void main() {
  ...
  group('BetaReadinessEngine', () {
    ...
    test('visible copy blocks avoid transcript text', () {
      ArchiveBetaMissionGate.enabledOverride = true;
      final blocks = BetaReadinessEngine.build().visibleCopyBlocks.join('\n').toLowerCase();
    ...
    test('engine source avoids seeding or fake journal entries', () {
      const enginePath = 'lib/features/beta_readiness/beta_readiness_engine.dart';
  ...
  group('TestingArchiveMeScreen', () {
    ...
    testWidgets('readiness tile appears when beta flag enabled', (tester) async {
      ArchiveBetaMissionGate.enabledOverride = true;
      await pumpScreen(tester);
      expect(find.byKey(const Key('testing_archiveme_beta_readiness_check')), findsOneWidget);
      expect(find.text(BetaReadinessCopy.openLink), findsOneWidget);
    });

    testWidgets('readiness tile hidden when beta flag disabled', (tester) async {
      ArchiveBetaMissionGate.enabledOverride = false;
      await pumpScreen(tester);
      expect(find.byKey(const Key('testing_archiveme_beta_readiness_check')), findsNothing);
    ...
    testWidgets('sheet opens with sections and warnings', (tester) async {
      ...
      expect(find.byKey(const Key('beta_readiness_check_sheet')), findsOneWidget);
      ...
      expect(find.text(BetaReadinessCopy.sectionReleaseWarnings), findsOneWidget);
      expect(find.text(BetaReadinessCopy.warningAppStoreProducts), findsOneWidget);
      expect(find.text(BetaReadinessCopy.warningLocalBackupPlainJson), findsOneWidget);
      expect(find.text(BetaReadinessCopy.warningArchiveLocalUnlessExport), findsOneWidget);
      expect(find.text(BetaReadinessCopy.itemExportLocalBackupAvailable), findsOneWidget);
      expect(find.text(BetaReadinessCopy.itemRestoreLocalBackupAvailable), findsOneWidget);

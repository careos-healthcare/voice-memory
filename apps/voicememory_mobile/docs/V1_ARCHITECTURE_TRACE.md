...
| **Display** | `ArchiveBeliefScreen`, evidence screens | `JournalStore` queries | Account-scoped reads | Archive lock / biometric gate | Lazy list reads | Empty states; no research dashboards in release | `v1_navigation_guard_test.dart`, archive widget tests |

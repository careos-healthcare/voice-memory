"use client";

import { useEffect, useState } from "react";
import Link from "next/link";
import { RefreshCw } from "lucide-react";

import { CallbackLearningDebugPanel } from "@/archived-components/_archived/debug/CallbackLearningDebugPanel";
import { SiteHeader } from "@/components/SiteHeader";
import { Button } from "@/archived-components/_archived/ui/button";
import { Card, CardContent } from "@/archived-components/_archived/ui/card";
import { buildCallbackLearningDebugReport } from "@/lib/debug/callback-learning-review";
import type { CallbackLearningDebugReport } from "@/types/callback-learning";

export default function CallbackLearningDebugPage() {
  const [report, setReport] = useState<CallbackLearningDebugReport | null>(null);

  const refresh = () => {
    setReport(buildCallbackLearningDebugReport());
  };

  useEffect(() => {
    refresh();
  }, []);

  return (
    <div className="min-h-screen bg-zinc-950">
      <div className="mx-auto max-w-5xl px-4 pb-20 sm:px-6">
        <SiteHeader />

        <header className="mt-2 flex items-start justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-violet-300/80">Retention</p>
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-white">
              Callback learning
            </h1>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-zinc-400">
              Local-only preference weights from opens, saves, ignores, and follow-up reflections.
              Adjustments stay internal — never shown as scores in product UI.
            </p>
          </div>
          <Button type="button" variant="ghost" size="sm" onClick={refresh}>
            <RefreshCw className="h-4 w-4" />
            Refresh
          </Button>
        </header>

        {!report ? (
          <Card className="mt-6">
            <CardContent className="py-12 text-center text-sm text-zinc-500">Loading…</CardContent>
          </Card>
        ) : (
          <div className="mt-6">
            <CallbackLearningDebugPanel report={report} />
          </div>
        )}

        <div className="mt-10 flex flex-wrap gap-3 text-sm">
          <Link href="/internal/resurfacing-confidence" className="text-violet-300 hover:text-violet-200">
            Resurfacing confidence →
          </Link>
          <Link href="/internal/callback-learning" className="text-zinc-500 hover:text-zinc-300">
            Callbacks →
          </Link>
          <Link href="/internal/return-triggers" className="text-zinc-500 hover:text-zinc-300">
            Return triggers →
          </Link>
        </div>
      </div>
    </div>
  );
}

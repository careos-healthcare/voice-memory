...
class AppConfig {
  ...
  static Future<void> initApiResolution() async {
    ...
    if (!physical) {
      ...
      _backendConfigured = true;
      _apiResolutionInitialized = true;
      AppLogger.debug('AppConfig: debug API base → $_resolvedApiBase');
      return;
    }
  ...
  static Future<bool> _isPhysicalDevice() async {
    ...
    return switch (info) {
      final AndroidDeviceInfo android => android.isPhysicalDevice,
  ...
  static String? _urlFromDartDefinesSync() {
    const backend = String.fromEnvironment('BACKEND_URL');

...
class BackendUrlResolver {
  ...
  static String? _fromDartDefines() {
    ...
    const primary = String.fromEnvironment(primaryDefineKey);
    if (primary.trim().isNotEmpty) return _normalize(primary.trim());

    const legacy = String.fromEnvironment(legacyDefineKey);
  ...
  static void _logSource(String source, String url) {
    if (kDebugMode) {
      AppLogger.debug('AppConfig: API base from $source → $url');
    }
  }
}
...
class ArchiveTheoryStrengthening {
  ...
  static ArchiveTheoryStrengthening fromEvidence({
    ...
    if (supporting < 3) {
      lines.add(
        'More reflections that name this pattern in your own words (at least three mentions).',
      );
    }
    if (split.stale) {
      lines.add(
        'A recent reflection that mentions this theme again in the latest part of your archive.',
      );
    }
    ...
    if (maxContradictionScore >= 60) {
      lines.add(
        'Fewer contradictions on the same theme, or reflections that reconcile the tension.',
      );
    }
    if (split.consistencyRatio < 0.55 && supporting >= 3) {
      lines.add(
        'Reflections that consistently point the same direction, not mixed signals.',
      );
    }

    if (lines.isEmpty) {
      lines.add(
        'Additional reflections over time that repeat or refine this theme.',
      );
    }

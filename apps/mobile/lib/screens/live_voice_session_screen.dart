...
/// Full-screen Gemini live conversation session.
...
class _LiveVoiceSessionScreenState extends State<LiveVoiceSessionScreen>
    ...
    try {
      ...
      if (TrialMode.enabled) {
        await ActivationTracker.trackEvent(
          const ArchiveEvent(OnboardingEvents.trialRecordingStarted),
        );
      }
  ...
  Future<void> _handleStopAndSave() async {
    ...
    if (TrialMode.enabled) {
      await ActivationTracker.trackEvent(
        const ArchiveEvent(OnboardingEvents.trialSaveStarted),
      );
    }
    try {
      ...
      if (!mounted) return;
      context.pop(result);
    } on CapturePipelineFailure catch (_, stackTrace) {
      if (!mounted) return;
      await _liveVoice.handleSessionFailure(
        LiveVoiceErrorState.unknown,
        reason: 'save_failed',

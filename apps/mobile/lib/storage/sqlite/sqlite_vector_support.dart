...
abstract final class SqliteVectorSupport {
  ...
  /// Initializes vector search on [Migration009ReflectionEmbeddings.embeddingsTable].
  static Future<void> initReflectionEmbeddingIndex(Database db) async {
    if (!ensureLoaded()) return;
    try {
      await db.execute(
        "SELECT vector_init('${Migration009ReflectionEmbeddings.embeddingsTable}', "
        "'embedding', "
        "'type=FLOAT32,dimension=${ReflectionEmbeddingContract.dimensions}')",
      );
    } on Object {
      // Extension unavailable — blob + Dart fallback remains.
    }
  }
}

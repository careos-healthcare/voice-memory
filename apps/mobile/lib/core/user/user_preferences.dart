...
class UserPreferences {
  const UserPreferences({
    ...
    this.isLedgerOptInEnabled = false,
  ...
  static const ledgerOptInPreferenceKey = 'ledger_opt_in_enabled';
  ...
  /// Separate from [isCloudSyncEnabled]. Encrypted sync does not turn this on.
  final bool isLedgerOptInEnabled;
  ...
  static Future<UserPreferences> load(MobilePrefsStore prefs) async {
    ...
    final ledger = await prefs.readBool(ledgerOptInPreferenceKey) ?? false;
  ...
  static Future<void> setLedgerOptInEnabled(
    MobilePrefsStore prefs,
    bool enabled,
  ) async {
    await prefs.writeBool(ledgerOptInPreferenceKey, enabled);
  }

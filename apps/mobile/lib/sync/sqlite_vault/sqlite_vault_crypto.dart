...
final class SqliteVaultCrypto {
  ...
  Uint8List sealDatabaseBytes(Uint8List databaseBytes) {
    ...
    final plaintext = Uint8List(databaseBytes.length + digestByteLength)
      ...
      ..setRange(databaseBytes.length, databaseBytes.length + digestByteLength, digest);

    final iv = _randomIv();
    final encrypter = Encrypter(
      AES(Key(_keyBytes), mode: AESMode.gcm),
    );
  ...
  Uint8List openSealedDatabaseBytes(Uint8List sealedBytes) {
    ...
    final databaseBytes =
        plaintext.sublist(0, plaintext.length - digestByteLength);
    final digest =
        plaintext.sublist(plaintext.length - digestByteLength, plaintext.length);

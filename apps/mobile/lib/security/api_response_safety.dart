...
abstract class ApiResponseSafety {
  ...
  static void ensureJsonResponse(http.Response response) {
    if (responseLooksLikeHtml(response)) {
      ...
      throw const FormatException(htmlResponseMessage);
    }
  }
  ...
  static Map<String, dynamic> decodeJsonObject(http.Response response) {
    ...
    if (decoded is! Map<String, dynamic>) {
      throw const FormatException('Expected JSON object from API');
    }
    return decoded;
  }

  /// Reads unified `{ error: { code, message, ... } }` or legacy `{ error, code }`.
  static ({String? message, String? code}) parseApiErrorFields(
    ...
    if (nested is String) {
      return (message: nested, code: body['code'] as String?);
    }
    return (message: null, code: body['code'] as String?);
  }

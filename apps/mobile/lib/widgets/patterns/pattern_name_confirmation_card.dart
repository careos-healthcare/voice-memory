...
class _PatternNameConfirmationCardState
  ...
  Future<void> _confirmYes() async {
    await PatternNameStore.confirm(widget.prompt.patternKey);
  ...
  Future<void> _openRename() async {
    final saved = await RenamePatternSheet.show(
      ...
      onSave: (name) async {
        await PatternNameStore.setCustomName(widget.prompt.patternKey, name);

...
# `flutter analyze` is run under `|| true`, so an analyzer that never started
# yielded all-zero counts. The script then died on `pipefail` from the empty
# `grep " info •"` below and printed nothing at all — a real failure that read
# as an unexplained one. Say so instead.
if (( errors == 0 && warnings == 0 && info_count == 0 )) \
   && ! grep -q "No issues found" "$OUTPUT"; then
  echo >&2
  echo "error: analyzer produced no parseable findings and no clean bill of health;" >&2
  echo "       treating as a failed analysis rather than a pass. Raw output:" >&2
  sed -e 's/^/       /' "$OUTPUT" >&2
  exit 1
fi

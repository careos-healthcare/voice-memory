...
# `flutter analyze` runs under `|| true`, so an analyzer that never started left
# both counts at zero and this printed a clean bill of health for work it had not
# done. The file-count guard above cannot catch that.
if ! grep -qE " (error|warning|info) •|No issues found" "$OUTPUT"; then
  echo >&2
  echo "error: analyzer produced no parseable output for ${#existing[@]} file(s);" >&2

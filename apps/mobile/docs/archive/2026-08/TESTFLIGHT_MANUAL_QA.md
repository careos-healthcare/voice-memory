...
| **Build number** | `48` (`pubspec.yaml` → `version: 0.2.0+48`) |
...
- `archiveme://` — primary deep link scheme.
...
### I2. Offline sync production evidence (release gate)

Required when sync is enabled in `release/focused_beta_status.json`. Run on the **same TestFlight build** you uploaded.

1. Settings → **About** → tap version label **7 times** to unlock developer tools.
2. Settings → **Offline sync verify**.
3. Airplane mode **ON** → record **5 eligible** moments → **Lock baseline** → force-quit → reopen → confirm local archive intact.
4. Airplane mode **OFF** → sign in if prompted → **Sync** → **Export evidence** (JSON to clipboard).
5. Save to `mobile/evidence/offline_sync_tested.json`; confirm `build_number` and `commit_sha` match this release.
6. `npm run validate:offline-sync-production` and update manifest gate `sync_offline_conflict`.

Full steps: [release/DEVICE_EVIDENCE_RUNBOOK.md](../../../release/DEVICE_EVIDENCE_RUNBOOK.md).
...
3. TestFlight build number matches `pubspec.yaml` build `48` (or incremented upload).
...
- [release/DEVICE_EVIDENCE_RUNBOOK.md](../../../release/DEVICE_EVIDENCE_RUNBOOK.md) — TestFlight + offline sync evidence

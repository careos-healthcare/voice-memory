...
class RemoteProcessingConsentStore {
  ...
  /// Raw consent record for [purpose] — **not** a permission to upload.
  ///
  /// This deliberately ignores the "Never send to server" setting, so it is
  /// the wrong predicate for any remote boundary. Call
  /// `RemoteProcessingConsentGate.isPurposePermittedNow` instead; a guard test
  /// (`test/privacy/remote_processing_consent_bypass_guard_test.dart`) fails
  /// when a new caller under `lib/` reaches for this method directly.

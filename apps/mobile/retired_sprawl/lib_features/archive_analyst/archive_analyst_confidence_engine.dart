...
class ArchiveAnalystConfidenceEngine {
  ...
  /// Exposes confidence math for developer inspection (X-Ray panel).
  ConfidenceScoreBreakdown breakdown({
    ...
    final rawBeforeModifiers =
        volume + consistency + recency - contradictionPenalty - counterPenalty;
    var raw = rawBeforeModifiers;
    final lowEvidence = supportingCount < 3;
    final staleApplied = stale;
    if (lowEvidence) {
      raw = (raw * 0.6).round();
    }
    if (staleApplied) {
    ...
    return ConfidenceScoreBreakdown(
      ...
      counterPenalty: counterPenalty,
      lowEvidenceMultiplierApplied: lowEvidence,
      staleMultiplierApplied: staleApplied,
      rawTotalBeforeModifiers: rawBeforeModifiers,
      finalPercent: raw.clamp(0, 100),
    );
  }
...
/// Component scores from [ArchiveAnalystConfidenceEngine.breakdown].

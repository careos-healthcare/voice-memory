...
import 'package:archiveme_mobile/features/patterns/transcript_evidence_extractor.dart';
...
abstract class EvidenceFirstPatternCopyResolver {
  ...
  static EvidenceFirstPatternCopyBundle _fallbackBundle(
    ...
    return const EvidenceFirstPatternCopyBundle(
  ...
  static void _log({
    ...
    AppLogger.debug('ARCHIVEME_PATTERN_COPY_SOURCE source=${source.name}');
  }

  static void _logFallback(String reason) {
    AppLogger.debug('ARCHIVEME_EVIDENCE_FIRST_COPY_FALLBACK reason=$reason');
    AppLogger.debug('ARCHIVEME_PATTERN_COPY_SOURCE source=fallback');
  }
}
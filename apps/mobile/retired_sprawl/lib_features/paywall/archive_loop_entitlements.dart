...
abstract class ArchiveLoopPaywallCopy {
  ...
  static const String privacyPolicyUrl = AppConfig.privacyUrl;
...
abstract class ArchiveLoopEntitlementLog {
  ...
  static void logAllowed(ArchiveLoopEntitlementFeature feature) {
    AppLogger.debug('ARCHIVEME_ENTITLEMENT_GATE_ALLOWED feature=${feature.id}');
  }

  static void logBlocked(ArchiveLoopEntitlementFeature feature) {
    AppLogger.debug('ARCHIVEME_ENTITLEMENT_GATE_BLOCKED feature=${feature.id}');
  }

  static void logProActive({required String source}) {
    AppLogger.debug('ARCHIVEME_ENTITLEMENT_PRO_ACTIVE source=$source');
  }
}
...
abstract class ArchiveLoopPaywallLog {
  ...
  static void logShown({
    ...
    AppLogger.debug('ARCHIVEME_PAYWALL_SHOWN trigger=$trigger$featurePart');
  }

  static void logCtaTapped() {
    AppLogger.debug('ARCHIVEME_PAYWALL_CTA_TAPPED');
  }

  static void logDismissed() {
    AppLogger.debug('ARCHIVEME_PAYWALL_DISMISSED');
  }
}

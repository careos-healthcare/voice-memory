...
extension RecordPostSaveCards on _RecordScreenState {
  List<Widget> _buildRecordPostSaveCards(
    ...
    return [
      if (ctx.ui == RecordUiState.done && ctx.entriesAfterSave.isNotEmpty) ...[
        if (ctx.justSavedFirstEntry &&
          ...
          PostSaveRecordedSummaryCard(
            ...
            primaryArchiveResult: PostSavePrimaryArchiveKind.firstEntryFootnote,
            onCorrectTranscript:
        ...
        if (!ctx.suppressNoisyFirstSaveCards &&
          ...
          PostSaveRecordedSummaryCard(
            ...
            showAnalysisPendingNote: false,
            mirror: ctx.postSaveDailyMirror!,
            primaryArchiveResult: ctx.postSaveArchiveHierarchy?.kind,
            ...
            onCorrectTranscript:
                ...
                : null,
            onAddMoreDetail: ctx.suppressLatestSaveArchiveInsight
                ? () => unawaited(
                ...
                : null,
            onBackToRecord: ctx.showDegradedTranscriptFocusedPostSave
          ...
          if (!ctx.suppressDegradedTranscriptPostSaveCompetitors) ...[
            if (MomentQualityFeedbackGates.shouldShow(
              ...
              hierarchyAllowsFeedback:
                  (ctx.postSaveArchiveHierarchy?.showMomentQualityFeedback ??
                      true) &&
            ...
            if (ctx.showFirstProofPayoff &&
              ...
              FirstProofPayoffCard(
                ...
                patternConfidence: ctx.firstProofPatternConfidence!,
                suppressCtas: ctx.firstProofActionLoopContent != null,
                ...
                onViewPatternDetails:
                    ctx.firstProofPayoffCandidate!.canShowPatternDetail
              ...
              if (ctx.showReturnAfterProofLiftV2OnPostSave) ...[
                ...
                ReturnAfterProofLiftV2Card(
                  result: ctx.returnAfterProofLiftV2PostSaveCandidate,
              ...
              if (ctx.showProUnderstandingLiftOnPostSave &&
                  ctx.proUnderstandingLiftPostSaveResult != null) ...[
                const SizedBox(height: 12),
                ProUnderstandingLiftCard(
                  result: ctx.proUnderstandingLiftPostSaveResult!,
                  onSeePro: () => _openProEvidenceValueSubscription(
                    analyticsSource: 'record_post_save_pro_understanding_lift',
                  ),
                ),
              ] else if (ctx.showProVisibilityLiftOnPostSave &&
                  ctx.proVisibilityLiftPostSaveResult != null) ...[
                const SizedBox(height: 12),
                ProVisibilityLiftCard(
                  result: ctx.proVisibilityLiftPostSaveResult!,
                  onSeePro: () => _openProEvidenceValueSubscription(
                    analyticsSource: 'record_post_save_pro_visibility_lift',
                  ),
                ),
                ...
                ProPreviewCard(
                  ...
                  ctx.proBridgeVisibilityPostSaveResult != null) ...[
                const SizedBox(height: 12),
                ProBridgeVisibilityCard(
                  result: ctx.proBridgeVisibilityPostSaveResult!,
                  onSeePro: () => _openProEvidenceValueSubscription(
                    analyticsSource: 'record_post_save_pro_bridge_visibility',
                  ),
                  onDismiss: () => unawaited(
                    _dismissProEvidenceValueBridge(),
                  ),
              ...
              if (ctx.showProofQualityResponseOnFirstProofPayoff) ...[
                ...
                ProofQualityResponseCard(
                  result: ctx.proofQualityResponseFirstProofCandidate,
                  source: 'record_post_save',
                  onChanged: () => setState(() {}),
                ),
                ...
                ProofSpecificityBoostCard(
                  result: ctx.proofSpecificityBoostPostSaveCandidate,
            ...
            if (ctx.showTimelineProofMomentOnFirstProofPayoff &&
                ctx.timelineProofMomentPostSaveCandidate != null) ...[
              const SizedBox(height: 12),
              TimelineProofMomentCard(
                result: ctx.timelineProofMomentPostSaveCandidate!,
              ...
              if (ctx.showBetaProofLiftUnderTimelineProofPostSave &&
                ...
                const SizedBox(height: 12),
                BetaProofLiftCard(
                  result: ctx.betaProofLiftTimelinePostSaveCandidate,
                  source: 'record_post_save_first_proof',
                  surface: 'record_post_save_first_proof',
                ),
              ],
              ...
              if (ctx.showProofQualityResponseOnTimelineProofPostSave) ...[
                const SizedBox(height: 12),
                ProofQualityResponseCard(
                  result: ctx.proofQualityResponseTimelinePostSaveCandidate,
                  source: 'record_post_save_first_proof',
                  onChanged: () => setState(() {}),
                ),
              ] else if (ctx
            ...
            if (ctx.showProofSpecificityOnFirstProofPayoff &&
                ctx.proofSpecificityPostSaveCandidate.shouldShow) ...[
              const SizedBox(height: 12),
            ...
            if (ctx.betaFeedbackIntelligenceSurfacePostSave != null &&
                ReturningRecordWatchTargetUiGates.showBetaRecordSurfaces()) ...[
              const SizedBox(height: 12),
              BetaFeedbackIntelligenceCard(
                surface: ctx.betaFeedbackIntelligenceSurfacePostSave!,
                entryCount: ctx.postSaveEntryCount,
                reachedFirstProof:
            ...
            if (ctx.showFirstProofTruth) ...[
              ...
              FirstProofTruthCard(
                ...
                entryCount: ctx.postSaveEntryCount,
                hasSnippets: ctx.firstProofPayoffCandidate!.hasSnippets,
                onAnswered: () {
                  if (mounted) setState(() {});
                },
              ),
            ...
            if (ctx.firstProofActionLoopContent != null) ...[
              ...
              FirstProofActionLoopCard(
                ...
                onWatchThisNext: _handleFirstProofWatchThisNext,
                onViewPatternDetails:
                    ctx.firstProofActionLoopContent!.canShowPatternDetails
                    ? _openFirstProofPatternDetail
                    : null,
                onRenamePattern:
                    ctx.firstProofActionLoopContent!.canRenamePattern
                    ? _openFirstProofRenamePattern
                    : null,
                onKeepRecording: _keepRecording,
                onCorrectTranscript:
                    ctx.firstProofActionLoopContent!.canCorrectTranscript
                    ? () {
                    ...
                    : null,
                onRemoveFromPattern:
                    ctx.firstProofActionLoopContent!.canRemoveFromPattern
                    ? () => unawaited(
                        _excludeLatestFromFirstProofPattern(),
                      )
                    : null,
                onOpenPatternCorrection:
                    ctx.firstProofActionLoopContent!.canShowPatternCorrection
                    ? () => unawaited(
            ...
            if (ctx.confirmedRepeatHelpfulActionPayoff != null) ...[
              const SizedBox(height: 16),
            ...
            if (ctx.confirmedRepeatChangeNotice != null) ...[
              ...
              ConfirmedRepeatChangeNoticeCard(
                ...
                onRecordWhatHelped: () {
                  ...
                  setState(
                    () => _selectedPromptLine =
                        ctx.confirmedRepeatChangeNotice!.guidedRecordPrompt,
                  );
                  _resetPostSaveToReady();
                },
                onViewEvidence: () => context.push(
            ...
            if (ctx.repeatReturnCheckOffer != null &&
              ...
              RepeatReturnCheckCard(
                ...
                entryCount: ctx.repeatReturnCheckOffer!.entryCount,
                surface: 'record',
                onChanged: () {
                  if (mounted) setState(() {});
                },
            ...
            if (ctx.postSaveArchiveHierarchy?.showMomentQualityFeedback ?? true)
              Builder(
                builder: (context) {
                  if (ctx.suppressLatestSaveArchiveInsight) {
                    return const SizedBox.shrink();
                  }
                  final returnTrigger = const CapacityReturnTriggerEngine()
                      .buildFromJournal(
                        entries: ctx.entriesAfterSave,
                        capacityLoopActive: _activeLoop?.isCapacityYes ?? false,
        ...
        if (ctx.postSaveArchiveHierarchy?.showBeliefUpdateCard == true &&
        ...
        if (ctx.postSaveArchiveHierarchy != null &&
            ctx.postSaveArchiveHierarchy!.showFocusedActionsBar &&
            !ctx.suppressDegradedTranscriptPostSaveCompetitors &&
        ...
        if (ctx.returnLoopPayoff != null &&
          ...
          DayTwoReturnLoopCard(
            ...
            onAddAnother: () => unawaited(_onRecordPressed(source: 'main')),
        ...
        if (ctx.suppressNoisyRepeatPostSaveCards &&
          ...
          RepeatPostSaveCard(
            ...
            onCorrectTranscript:
                ...
                : null,
            onViewThoughtMap:
                ctx.repeatPostSaveThoughtMapPreview?.shouldShow == true
        ...
        if (_saveReceipt != null &&
          ...
          StartHereSaveReceiptCard(
            ...
            onDismiss: () => setState(() => _saveReceipt = null),
          ),
        ...
        if (_doneForTodayReceipt != null &&
          ...
          Builder(
            builder: (context) {
              final path = const TwoDayActivationEngine().buildPostSave(
        ...
        if (!ctx.stack.showInputQualityCoach &&
          ...
          ReturnDayClosedCard(
            ...
            nextCheck: _completedCheckInToday!.tomorrowsBetterQuestion,
            onDone: () => setState(
          ...
          if (ctx.stack.showArchiveProofCards && _patternProgress != null) ...[
            const SizedBox(height: 16),
          ...
          CheckInCompletedCard(
            ...
            languageCode: _languageCode,
            betterResultIntensity: ScreenshotMode.screenshotBetterResult
            ...
            nextCheckSlot: ctx.stack.showResultNextCheck
                ? ResultNextCheckCard(
                    ...
                    routineAnchorPicker: ctx.stack.showRoutineAnchor
                        ? () => RoutineAnchorChooser.show(
                            context,
                          )
                        : null,
                    onRoutineAnchorChosen: ctx.stack.showRoutineAnchor
                        ? (anchor) => RoutineAnchorStore.instance().saveForDate(
                            _tomorrowDateKey,
                            anchor,
                          )
                        : null,
                    onCreateCheckIn: (question) async {
                      ...
                      final anchor = await RoutineAnchorStore.instance()
                          .loadForDate(
                            _tomorrowDateKey,
                          );
                      final active =
          ...
          if (shouldShowKinderAngle(
            _inputQualityText,
            resultHint: _completedCheckInToday!.selectedOptionId ?? 'same',
          ))
            KinderAngleCard(
          ...
          if (ctx.stack.showArchiveProofCards && _patternMemory != null) ...[
            const SizedBox(height: 16),
          ...
          if (ctx.stack.showArchiveProofCards &&
            ...
            HabitProofCard(
              proof: _habitProof!,
              onKeepGoing: () => _keepHabitProofGoing(_habitProof!),
            ),
          ],
          if (ctx.stack.showArchiveProofCards && _weeklyRecap != null) ...[
            const SizedBox(height: 16),
            WeeklyPatternRecapCard(
              recap: _weeklyRecap!,
              onUseNext: ctx.suppressPostResultNextCheckCompetitors
                  ? null
                  : () => _useWeeklyRecapNext(_weeklyRecap!),
            ),
          ],
          if (ctx.stack.showArchiveProofCards && _shareRecap != null) ...[
            Align(
        ...
        if (!ctx.stack.showInputQualityCoach &&
          ...
          if (_secondSessionComparison?.hasEnoughData == true &&
            ...
            SecondSessionComparisonCard(
              ...
              onGoDeeper: () {
                final prompt = _secondSessionComparison!.whatToTestNext;
          ...
          if (_postSavePattern != null) ...[
            ...
            PostSaveInsightChoiceCard(
                ...
                if (_isFirstSessionPostSave) {
                  final thread =
                      await FirstSessionCoordinator.acceptForTomorrow(
                        pattern,
                        reflectionText: _lastSavedEntry?.transcript ?? '',
              ...
              onRecordNext: _keepRecording,
              onRecordNextEvidence: (prompt) => _keepRecording(

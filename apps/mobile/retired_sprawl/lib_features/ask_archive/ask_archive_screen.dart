...
import 'package:archiveme_mobile/features/archive_search/archive_search_engine.dart';
...
class AskArchiveScreen extends StatefulWidget {
  ...
  /// Hook for Pro gating in tests. Defaults to [ArchiveEntitlementReader].
...
class _AskArchiveScreenState extends State<AskArchiveScreen> {
  ...
  Future<bool> Function()? get _entitlementReader =>
      widget.entitlementReader ??
      () => ArchiveEntitlementReader.forAccessCheck().isPro;

  @override
  void initState() {
    super.initState();
    if (!BetaSurfacesFeatureFlags.askArchive && !ScreenshotMode.enabled) {
  ...
  Future<List<KeyMoment>> _momentsForSearch({
    ...
    final trigger = await PaywallAccess.check(
      ...
      sourceRoute: RouteCatalog.askArchive,
    );
  ...
  void _searchSuggested(ArchiveSuggestedSearch suggested) {
    ...
    unawaited(_runSearch(suggested.query, fromSuggestedChip: true));
  }
  ...
  void _openMoment(ArchiveSearchResult result) {
    ...
    if (!moment.id.startsWith('km_')) {
      unawaited(context.push('/entry/${moment.id}'));
      return;
    }
  ...
  void _useCheck(ArchiveSearchResult result) {
    ...
    if (result.nextCheck case final check?) {
      context.go(RouteCatalog.recordHome, extra: {'prefillCheck': check});
    }
  }

...
import 'package:archiveme_mobile/features/live_audio/application/live_voice_telemetry_source.dart';
...
import 'package:archiveme_mobile/features/live_audio/domain/models/live_voice_error_messages.dart';
...
class LiveVoiceCaptureService implements Listenable, LiveVoiceTelemetrySource {
  ...
  Future<void> start({
    ...
    try {
      ...
      LiveAudioPipelineLog.diagnostics(
        'capture started pipeline=$shouldUsePipeline $diagnostics',
      );
      _emitDiagnostics();
  ...
  Future<CapturePipelineResult> stopAndSave() async {
    if (!_active) {
    ...
    _notifyErrorState();
    _startedAt = null;
    LiveAudioPipelineLog.diagnostics('stopAndSave $diagnostics');
    ...
    return (await _pipeline.saveLiveVoiceTranscript(
  ...
  Future<void> cancel() async {
    ...
    _startedAt = null;
    _transcripts.reset();
    LiveAudioPipelineLog.diagnostics('cancel $diagnostics');
  }

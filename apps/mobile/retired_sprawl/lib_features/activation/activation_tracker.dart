...
abstract class ActivationTracker {
  ...
  /// product analytics when not in trial mode.
  static Future<void> trackEvent(ArchiveEvent event) async {
    if (!AppServices.isInitialized) return;
    if (!V1EventRegistry.allowed.contains(event.name)) return;
    await _safe(() async {
      await _incrementEvent(event.name);
      _trackAnalytics(event.name, event.payload);
    });
  }

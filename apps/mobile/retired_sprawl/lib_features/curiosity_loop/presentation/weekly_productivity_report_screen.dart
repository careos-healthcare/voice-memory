...
import 'package:archiveme_mobile/features/curiosity_loop/presentation/widgets/connected_clinical_telemetry_trend_widget.dart';
...
class _WeeklyProductivityReportScreenState
  ...
  Future<void> _exportMarkdownDigest() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(WeeklyProductivityReportCopy.markdownCopiedToast),
  ...
  Widget _buildBody(BuildContext context, WeeklyProductivityReport report) {
    if (report.totalReactions == 0) {
      return const _EmptyState(key: Key('weekly_productivity_report_empty'));
    }
      ...
      child: Column(
        ...
        children: [
          ...
          OutlinedButton(
            ...
            child: const Text(WeeklyProductivityReportCopy.exportMarkdownLabel),
          ),
          const SizedBox(height: AppSpacing.sm),
          OutlinedButton(
            key: const Key('weekly_productivity_report_export_json'),
            onPressed: exporting ? null : onExportJson,
            child: const Text(WeeklyProductivityReportCopy.exportJsonLabel),
          ),
        ],
      ),
    );

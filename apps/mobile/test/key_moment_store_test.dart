...
void main() {
  test('save then loadAll returns newest first', () async {
    ...
    await store.save(_moment('a', DateTime(2026, 6)));
  ...
  test('save de-duplicates by id', () async {
    ...
    await store.save(_moment('a', DateTime(2026, 6), text: 'first'));
    await store.save(_moment('a', DateTime(2026, 6), text: 'second'));
  ...
  test('caps the list at 500 newest moments', () async {
    ...
    for (var i = 0; i < 505; i++) {
      await store.save(
        _moment('m$i', DateTime(2026).add(Duration(days: i))),
      );
    }
    final all = await store.loadAll();

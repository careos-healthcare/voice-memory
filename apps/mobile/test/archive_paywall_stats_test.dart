import 'package:archiveme_mobile/billing/archive_intelligence_proof.dart';
...
ArchiveV1View _viewWithBelief(List<JournalEntry> entries) {
...
void main() {
  ...
  test('variant B pre-cta lists patterns theories contradictions', () {
    final entries = List.generate(
      ...
      (i) => _entry(
        'e$i',
        DateTime.utc(2025, 12).add(Duration(days: i)),
        const ['work', 'stress'],
      ),
    );
    final stats = ArchivePaywallStats.fromEntries(
      entries: entries,
      archiveV1: _viewWithBelief(entries),
    );
  ...
  test('hero lines for variant B', () {
    final entries = List.generate(
      ...
      (i) => _entry(
        'e$i',
        DateTime.utc(2025).add(Duration(days: i * 4)),
        const ['work'],
      ),
    );
  ...
  test('uses fallback pre-cta when few recordings', () {
    final stats = ArchivePaywallStats.fromEntries(
      entries: [
        _entry('e1', DateTime.utc(2026), const ['work']),
      ],
    );
    expect(
  ...
  test('proof fallback when no themes theories or changes', () {
    final stats = ArchivePaywallStats.fromEntries(
      entries: [_entry('e1', DateTime.utc(2026), const [])],
    );
    final proof = ArchiveIntelligenceProofView.fromStats(stats);

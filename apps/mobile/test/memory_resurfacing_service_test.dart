...
void main() {
  ...
  test(
    'selectByAnniversary includes same month/day from previous years',
    () async {
  ...
  test(
    'selectByAnniversary does not consult or write store shown-tracking',
    () async {
  ...
  test(
    'selectByAnniversary returns empty for missing or unmatched entries',
    () async {
  ...
  test(
    'selectByAnniversary attaches belief relation only when genuinely related',
    () async {
      final dir = await Directory.systemTemp.createTemp(
        'vm_anniversary_belief',
      );
      final prefs = await MobilePrefsStore.open('${dir.path}/prefs.json');
      final service = MemoryResurfacingService(MemoryResurfacingStore(prefs));
      final now = DateTime(2026, 5, 18);

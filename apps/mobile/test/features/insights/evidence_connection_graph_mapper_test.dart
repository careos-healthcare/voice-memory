...
void main() {
  ...
  test('maps a reply to a center message node and one evidence node per entry',
      () async {
    final entries = {
      'entry-a': _entry(
        id: 'entry-a',
  ...
  test('missing or deleted entries become unavailable evidence nodes', () async {
    final graph = await mapper.map(
  ...
  test('trims message content for the center label without hypothesis copy',
      () async {
    const long =

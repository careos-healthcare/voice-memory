...
void main() {
  ...
  group('PricingValidationAnalytics', () {
    test('price and reason analytics are metadata only', () async {
      ...
      PricingValidationAnalytics.captureForTest = (event, props) {
      ...
      final priceProps =
          captured[PricingValidationAnalytics.priceSelectedEvent];
      expect(priceProps!['selected_price'], '4.99');
      expect(priceProps['source'], 'test');
      expect(priceProps['entry_count'], 4);
      expect(priceProps['has_useful_proof'], isTrue);
      expect(priceProps['active_repair_mode'], 'pricing_validation');
      expect(priceProps.containsKey('journal'), isFalse);

      final reasonProps =
          captured[PricingValidationAnalytics.reasonSelectedEvent];
  ...
  group('PricingValidationCard', () {
    ...
    testWidgets('price selection emits analytics', (tester) async {
      ...
      expect(
        captured.containsKey(PricingValidationAnalytics.priceSelectedEvent),
        isTrue,
      );
    });
  });

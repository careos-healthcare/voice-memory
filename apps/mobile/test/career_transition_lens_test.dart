...
void main() {
  test(
    'career transition lens exposes listen targets and cold-start prompt',
    () {
      ...
      expect(
        CareerTransitionLens.coldStartPromptSeeds.first,
        CareerTransitionLens.primaryColdStartPrompt,
      );
    },
  );
  ...
  test(
    'system prompt injection covers identity, skills, risk, and success',
    () {
  ...
  test('matches career transition lens only', () {
    expect(
      CareerTransitionLens.matches(LifeStageLens.careerTransition),
      isTrue,
    );

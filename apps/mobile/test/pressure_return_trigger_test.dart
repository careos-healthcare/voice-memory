...
import 'package:archiveme_mobile/widgets/pressure_retention/pressure_return_trigger_reminder.dart';
...
_MemoryExperimentStore _experimentForEntryCount(int count) {
  if (V1RetentionPolicy.requireEvidenceAnchoredHooks) {
    return _MemoryExperimentStore(acceptedFlag: true);
  }
  return _MemoryExperimentStore(acceptedFlag: count >= 5);
}
...
void main() {
  ...
  group('Return trigger engine', () {
    ...
    test('trigger appears when 5+ entries only outside V1 evidence mode', () {
  ...
  group('Pressure Insights integration', () {
    testWidgets('trigger card shows at 5+ entries and accept persists', (
      ...
      await _pumpInsights(
        ...
        records: _records(5),
        triggerStore: store,
        experimentStore: _experimentForEntryCount(5),
      );
  ...
  group('Record screen reminder', () {
    test('shows only once accepted and past the first session', () {
      final expectedVisible = V1RetentionPolicy.showCapacityReturnTriggers;
      expect(

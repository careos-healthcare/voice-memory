...
import 'package:archiveme_mobile/features/bulk_actions/bulk_archive_action_service.dart';
...
import 'package:archiveme_mobile/widgets/export/export_selected_sheet.dart';
...
void main() {
  ...
  group('Select + bulk flows on pinned evidence', () {
    ...
    Future<void> chooseBulkAction(WidgetTester tester, String id) async {
      final tile = tester.widget<ListTile>(find.byKey(Key('bulk_action_$id')));
      await tester.runAsync(() async {
        tile.onTap!();
        await tester.pump();
        await tester.pump(const Duration(milliseconds: 350));
    ...
    testWidgets('export sheet exports only the selected entries', (
      ...
      final selected = [_entry(id: 'a', transcript: 'Pinned reflection alpha')];
      String? captured;
      String? capturedName;
      await tester.pumpWidget(
        MaterialApp(

...
void main() {
  test(
    'parses monthly V2 with theory sections and surprise/contradiction',
    () {
      final review = ArchiveMonthlyReview.fromJson({
        ...
        'emergingTheories': [_conclusion('et1')],
        'fadingTheories': [],
      ...
      expect(review.emergingTheories.single.id, 'et1');
      expect(review.evidenceFor.single.id, 'ef1');
    },
  );

  test('parses milestone review without theory summary', () {
    ...
    expect(review?.primaryTheorySummary, isNull);
  });

  test('parses milestone review with theory summary', () {
    final review = ArchiveMilestoneReview.fromJson({
      'milestoneThreshold': 100,
      'eligibleCount': 100,
      'archiveHash': 'h',
      ...
      'headline': 'The first 100 reflections reveal…',
      'narrative': 'The archive weighed recurring work stress.',
      'primaryTheorySummary': _conclusion('pt1'),
      'changeHighlights': [_conclusion('c1')],
      'uncertaintyNote': 'More recordings could shift this.',
    });
    expect(review?.primaryTheorySummary?.id, 'pt1');
  });
}

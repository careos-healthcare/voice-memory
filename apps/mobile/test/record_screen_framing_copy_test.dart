...
void main() {
  ...
  group('RecordScreenFramingCopy', () {
    test('uses concrete first-recording guidance', () {
      expect(RecordScreenFramingCopy.title, ConsumerUiCopy.recordTitle);
      expect(RecordScreenFramingCopy.guidance, ConsumerUiCopy.recordSubtitle);
    });
  ...
  group('Record screen unified CTA policy', () {
    ...
    testWidgets('first-entry post-save shows one calm saved card', (
      ...
      expect(V1MomentSaveReceiptExpectations.transcript, findsOneWidget);
      expect(
    ...
    testWidgets('entry 3 post-save hides return handoff', (tester) async {
      ...
      expect(find.text(FirstProofPayoffCopy.headline), findsNothing);
      expect(find.text(FirstProofPayoffCopy.patternLine), findsNothing);
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        V1MomentSaveReceiptExpectations.expectDegradedFallbackVisible();
        expect(
          find.byKey(const Key('post_save_return_handoff_card_afterFirstSave')),
          findsNothing,
        );
        ...
        expect(find.text(ConsumerUiCopy.startRecording), findsNothing);
        expect(find.text(ConsumerUiCopy.doneCta), findsNothing);
        expect(find.text(VoiceCaptureCopy.recordAgainCta), findsNothing);
      },
    );
    ...
    testWidgets('fourth related post-save shows What changed v2 question', (
      ...
      expect(find.text(WhatChangedV2Copy.question), findsNothing);
      expect(
    ...
    testWidgets('V1 post-save hides What changed v2 interaction stack', (
      tester,
    ) async {
      await seedConfirmedRepeatEntries(tester, 4);
      VisualAuditOverrides.setRecordPresentation(
    ...
    testWidgets(
      'five confirmed-repeat entries stay capture-first without archive stack',
      (tester) async {
        await tester.runAsync(
          () => LowFrictionReturnStore.resetForTest(AppServices.instance.prefs),
        );

...
void main() {
  ...
  test(
    'readOutgoingEdges returns edges for the given source, ordered by weight descending',
    () async {
  ...
  test(
    'readOutgoingEdges only returns edges where the entry is the source, not the target',
    () async {
      final db = await openTestAppSqliteDatabase();
      final repository = EntryEdgesRepository(db);
  ...
  test(
    'readOutgoingEdges returns an empty list for an entry with no edges',
    () async {
      final db = await openTestAppSqliteDatabase();
      final repository = EntryEdgesRepository(db);
  ...
  test(
    'readOutgoingEdges returns an empty list for an empty source id',
    () async {
      final db = await openTestAppSqliteDatabase();
      final repository = EntryEdgesRepository(db);

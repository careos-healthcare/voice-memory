...
import 'package:archiveme_mobile/features/first_session/first_save_rescue.dart';
...
import 'package:archiveme_mobile/widgets/first_session/first_save_rescue_card.dart';
import 'package:archiveme_mobile/widgets/first_session/first_session_explanation_card.dart';
import 'package:archiveme_mobile/widgets/first_session/two_day_activation_card.dart';
...
void main() {
  ...
  group('Funnel tracker', () {
    ...
    test('archive share action carries only safe share metadata', () {
      ActivationFunnelAnalytics.track(
      ...
      expect(payload['share_type'], 'copy');
      expect(payload['status'], 'copied');
      expect(payload['memory_scope'], 'automatic');
    ...
    test('event names match the funnel spec and contain no VoiceMemory', () {
      const names = [
        ...
        ActivationFunnelAnalytics.passwordResetRequested,
      ...
      expect(names, [
        ...
        'referral_invite_seen',
        'referral_invite_copied',
        'referral_invite_dismissed',
        'referral_invite_link_copied',
        'invite_attribution_received',
        'invited_user_welcome_seen',
        'invited_user_welcome_tapped',
        'invited_user_first_save',
        'invited_app_opened',
        ...
        'invited_paywall_seen',
        'invited_purchase_started',
        'invited_purchase_completed',
        'invited_day_2_copy_seen',
        'invited_day_2_copy_tapped',
        'referral_proof_moment_seen',
        'review_prompt_seen',
        'review_prompt_tapped',
    ...
    test(
      'paywall and purchase funnel ids still match the attribution flow',
      () {
  ...
  group('Cards fire seen events', () {
    testWidgets('first-session card', (tester) async {
      ...
      expect(firstPayload(ActivationFunnelAnalytics.firstSessionCardSeen), {
        'entry_count': 0,
      });
    });
    ...
    testWidgets('day 1 plan fires stage day_1 and no day-2 event', (
      ...
      expect(firstPayload(ActivationFunnelAnalytics.twoDayActivationSeen), {
        'stage': 'day_1',
      });
    ...
    testWidgets('first save rescue card fires seen once, tap fires tapped', (
      ...
      expect(firstPayload(ActivationFunnelAnalytics.firstSaveRescueSeen), {
        'entry_count': 0,
      });
      ...
      expect(firstPayload(ActivationFunnelAnalytics.firstSaveRescueTapped), {
        'entry_count': 0,
      });
    });
    ...
    testWidgets('done for today receipt', (tester) async {
      ...
      expect(firstPayload(ActivationFunnelAnalytics.doneForTodaySeen), {
    ...
    testWidgets('value cards fire their seen events', (tester) async {
      ...
      final counter = const ArchiveProofCounterEngine().build(
        records,
        now: _base,
      );
      final review = const WeeklyThreadReviewEngine().build(
        records,
        now: _base,
      );
      ...
      final belief = const BeliefDistanceEngine().build([
        ...
        _record(id: 'b1', fear: 'Checking messages again tonight'),
      ], entryCount: 2);
      await pumpCard(
        tester,
        Column(
      ...
      expect(firstPayload(ActivationFunnelAnalytics.threadReturnEvidenceSeen), {
        'entry_count': 3,
        'has_connected_thread': 1,
      });
      expect(firstPayload(ActivationFunnelAnalytics.archiveProofCounterSeen), {
        'has_connected_thread': 1,
      });
      expect(
    ...
    testWidgets('value moment bridge fires seen and tapped', (tester) async {
      final bridge = const ValueMomentPaywallTrigger().build(
        _workThread3(),
        isPro: false,
        now: _base,
      );
      await pumpCard(
        tester,
        ValueMomentProBridge(bridge: bridge, onSeePro: () {}, onDismiss: () {}),
      );
      expect(
  ...
  group('Save funnel', () {
    test('first_recording_saved fires only for the very first save', () async {
      ...
      expect(firstPayload(ActivationFunnelAnalytics.firstRecordingSaved), {
        'entry_count': 1,
      });
    ...
    test(
      'first_save_rescue_saved fires only for a rescue-started first save',
      () async {
        addTearDown(FirstSaveRescue.resetForTest);
        ...
        final plainDir = Directory.systemTemp.createTempSync(
          'vm_rescue_plain_',
        );

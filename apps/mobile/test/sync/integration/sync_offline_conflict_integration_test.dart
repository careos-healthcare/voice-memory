...
void main() {
  ...
  group('Sync offline partition + conflict integration', () {
    ...
    test(
      'queues outbox mutations while offline and drains on reconnect',
      () async {
        ...
        expect(
          harness.syncApi.pushedSnapshots.last.single.transcript,
    ...
    test(
      'remote-ahead concurrent edit resolves via revision OCC on reconnect',
      () async {
        await harness.journal.save(
          _entry(id: 'conflict-a', transcript: 'shared'),
        );
    ...
    test(
      'local-ahead stale remote pull does not regress pending local edits',
      () async {
    ...
    test(
      'field-level merge keeps equal fields when remote revision is ahead',
      () async {
        await harness.journal.save(
    ...
    test('transient push failures recover via in-process retry', () async {
      await harness.journal.save(
        _entry(id: 'retry-a', transcript: 'payload v1'),
      );
    ...
    test(
      'remote tombstone propagates through merge and sqlite mirror',
      () async {
        ...
        final tombstone = await harness.journal.getByIdIncludingTombstones(
          'tomb-a',
        );
    ...
    test(
      'full cycle: offline edit, remote concurrent change, reconnect, mirror sync',
      () async {

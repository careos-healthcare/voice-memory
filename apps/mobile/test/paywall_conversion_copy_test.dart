  ...
  group('Paywall above-fold clarity', () {
    ...
    test(
      ...
      () {
        ...
        final stickyIdx = proPaywall.indexOf('PaywallStickyCheckoutBar(');
        expect(packagingIdx, greaterThan(-1));
        expect(generalClarityIdx, greaterThan(-1));
        expect(stickyIdx, greaterThan(-1));
        expect(
        ...
        expect(
          paywallBody
              .substring(
                packagingIdx,
                paywallBody.indexOf('extraScrollContent:', packagingIdx),
              )
              .contains('_generalConversionClaritySection('),
          isFalse,
          reason: 'packaging path must not render the clarity block',
        );
        expect(
          paywallBody.indexOf('PlanSelectionConfidenceBlock('),
          greaterThan(generalClarityIdx),
          reason: 'plan confidence follows clarity in scroll content',
        );
      },
    );
  ...
  group('Price confidence', () {
    ...
    test(
      ...
      () {
        ...
        expect(
          paywallBody.indexOf('PlanSelectionConfidenceBlock('),
          lessThan(paywallBody.indexOf('_priceConfidenceLines(),')),
          reason: 'plan confidence precedes price confidence in scroll content',
        );
        expect(
          paywallBody.indexOf('_priceConfidenceLines(),'),
          greaterThan(-1),
          reason: 'price confidence remains in scroll content',
        );
        expect(
          proPaywall,
          contains('confirmLine: widget.confirmLine'),
          reason: 'App Store confirm line is passed to sticky checkout',
        );
        expect(
          proPaywall,
          contains('PaywallSubscriptionDetailsSection('),
          reason: 'subscription details render in scroll content',
        );
      },
    );
  ...
  group('Purchase completion reassurance', () {
    ...
    test(
      ...
      () {
        ...
        expect(
          paywallBody.indexOf('_confidenceSection()'),
          greaterThan(-1),
          reason: 'reassurance block remains in purchase scroll content',
        );
        expect(
          paywallBody.indexOf('_confidenceSection()'),
          lessThan(paywallBody.indexOf('PlanSelectionConfidenceBlock(')),
          reason: 'reassurance precedes plan confidence in scroll content',
        );
        expect(
          proPaywall,
          contains('PaywallSubscriptionDetailsSection('),
        ...
        expect(
          ...
          reason: 'purchase CTA lives in the sticky checkout bar',
        );
      },
    );

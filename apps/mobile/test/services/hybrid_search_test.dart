...
void main() {
  ...
  group('MemoryTranscriptSearchRepository', () {
    ...
    test('hybridSearch fuses keyword and vector channels', () async {
      await journalRepo.mirrorEntireRemoteState([
      ...
      final hits = await searchRepo.hybridSearch(
  ...
  group('HybridSearchEngine', () {
    ...
    test('hybrid fusion boosts entries strong in both channels', () async {
      ...
      // Pins the vector leg on. It is off in the shipped engine because the
      // encoder that fills it is a stand-in — see [SemanticVectorFusion] — but
      // the fusion machinery itself still has to work for the day a real
      // encoder ships, which is what this case covers.
      final fused = HybridSearchEngine(
        repository: searchRepo,
        vectorFusionEnabled: true,
      );
      final hits = await fused.search(
        keywordQuery: 'budget planning',
        queryEmbedding: queryEmbedding,
        limit: 3,
      );
    ...
    test('the shipped engine leaves BM25 ranking alone', () async {
      await journalRepo.mirrorEntireRemoteState([
        _entry(id: 'both', transcript: 'budget planning for the quarter'),
        _entry(id: 'keyword-only', transcript: 'budget spreadsheet review'),
        _entry(id: 'vector-only', transcript: 'unrelated transcript text'),
      ...
      expect(
        ...
        isNot(contains('vector-only')),
        reason: 'a nearest-neighbour hit must not enter a keyword result set',
      );

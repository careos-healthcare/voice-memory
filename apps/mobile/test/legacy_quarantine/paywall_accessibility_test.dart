...
void main() {
  ...
  group('Paywall plan selector semantics (source contract)', () {
    ...
    test(
      'plan chips expose button/selected semantics in the sticky checkout bar',
      () {
        final source = File(
        ...
        expect(source, contains("hint: selected ? null : 'Selects this plan'"));
      },
    );
    ...
    test('a load error is announced via a live region', () {
      ...
      expect(source, contains('widget.errorMessage!'));
    });
  });
}
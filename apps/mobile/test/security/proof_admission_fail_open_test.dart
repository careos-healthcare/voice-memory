...
void main() {
  test(
    'ProofSourceEntry and ProofAdmissionCache default remoteProcessingConsented to false',
    () {
      final models = File(
        'lib/features/proof_admission/proof_admission_models.dart',
      ).readAsStringSync();
      final cache = File(
        'lib/features/proof_admission/proof_admission_cache.dart',
      ).readAsStringSync();
      ...
      expect(cache, contains('remoteProcessingConsented = false'));
    },
  );

  test(
    'capture pipeline saveRecoveredVaultEntry requires explicit consent param',
    () {
      final pipeline = File(

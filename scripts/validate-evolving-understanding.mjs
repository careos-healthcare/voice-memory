...
const copy = read("lib/product/evolving-understanding-copy.ts");
...
const whatNext = read("components/blind-spots/WhatHappensNextPanel.tsx");
if (!whatNext.includes("WHAT_HAPPENS_NEXT")) fail("WhatHappensNextPanel must use copy");

const card = read("components/theories/EvolvingViewCard.tsx");
const snapshot = read("lib/theories/evolving-view-snapshot.ts");
for (const phrase of [
...
const events = read("lib/metrics/evolving-understanding-events.ts");
for (const name of [
...
const reportSrc = read("lib/metrics/evolving-understanding-report.ts");
for (const field of [
...
const ladder = read("lib/theories/personal-theory-status.ts");
...
const intro = read("lib/product-copy.ts");
...
const forbidden = [
  blindCopy,
  read("lib/product/evolving-understanding-copy.ts"),
  paywall,
].join("\n");
...
const review = read("components/blind-spots/BlindSpotReview.tsx");
...
if (!pkg.includes("validate:evolving-understanding")) {
  fail("package.json missing validate:evolving-understanding");
}
...
} = await import("../lib/metrics/evolving-understanding-events.ts");

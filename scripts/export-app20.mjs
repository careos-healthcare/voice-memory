...
function classifyApp20(rel) {
  ...
  if (
    ...
    p.startsWith("app/internal/archive-moat") ||
  ...
  if (
    ...
    p.startsWith("components/recording/") ||
  ...
  if (
    ...
    p.startsWith("lib/distribution/") ||
    p.startsWith("components/distribution/") ||
    p.startsWith("app/internal/distribution") ||
    p.startsWith("lib/paywall") ||
    p.startsWith("types/onboarding") ||
    p.startsWith("types/retention") ||
    p.startsWith("types/distribution") ||
...
async function loadReports() {
  try {
    ...
    const { buildDistributionReport } = await import(
      path.join(ROOT, "lib/internal/distribution-report.ts")
    );
    const { buildArchiveMoatReport } = await import(
      path.join(ROOT, "lib/internal/archive-moat-report.ts")
    );
    return {

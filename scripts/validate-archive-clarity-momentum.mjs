...
const required = [
  "apps/web/components/archive/ArchiveHomeScore.tsx",
  "apps/web/components/archive/ReflectionImpactReceipt.tsx",
  "apps/web/components/archive/ArchiveProgressBar.tsx",
  "apps/web/components/archive/ArchiveIdentityBar.tsx",
  "apps/web/components/archive/WhyOpenArchiveToday.tsx",
  "apps/web/components/archive/ArchiveReductionSections.tsx",
  "packages/shared/lib/product/simplicity-mode.ts",
  "packages/shared/lib/archive/archive-reduction-rules.ts",
  "packages/shared/lib/archive/reflection-impact-receipt.ts",
  "packages/shared/lib/archive/archive-home-score.ts",
  "packages/shared/lib/archive/archive-case-file-progress.ts",
  "packages/shared/lib/archive/why-open-archive-today.ts",
...
const impact = read("packages/shared/lib/archive/reflection-impact-receipt.ts");
const successCopy = read("packages/shared/lib/design/archive-success-copy.ts");
...
const identityBar = read("apps/web/components/archive/ArchiveIdentityBar.tsx");
if (!identityBar.includes("VOICEMEMORY_ARCHIVE_POSITIONING")) {
  fail("ArchiveIdentityBar must use canonical positioning");
}
if (!fs.existsSync(path.join(ROOT, "packages/shared/lib/product/archive-positioning.ts"))) {
  fail("missing lib/product/archive-positioning.ts");
}

const discoverPage = read("apps/web/app/discover/page.tsx");
if (!discoverPage.includes("ArchiveBeliefHeader")) {
  fail("discover must render ArchiveBeliefHeader");
}
for (const page of ["apps/web/app/account/page.tsx", "apps/web/app/search/page.tsx", "apps/web/app/page.tsx"]) {
...
if (!read("packages/shared/lib/archive/archive-reduction-rules.ts").includes("Show more archive detail")) {
  fail("archive-reduction-rules must use Show more archive detail label");
}
if (!read("apps/web/components/archive/EvidenceArchiveHome.tsx").includes("ArchiveDetailHub")) {
  fail("archive home must include ArchiveDetailHub");
}

const whyOpen = read("packages/shared/lib/archive/why-open-archive-today.ts");
for (const line of [
...
const mobileShell = read("apps/mobile/lib/widgets/main_shell.dart");
...
const returning = read("packages/shared/lib/product/returning-home.ts");

...
const required = [
  "types/a-tier-prioritization.ts",
  "lib/blind-spots/a-tier-prioritization.ts",
  "lib/blind-spots/a-tier-quality-dashboard.ts",
  "components/blind-spots/EvidenceQualityBadge.tsx",
  "components/internal/ATierQualityDashboardPanel.tsx",
];
...
const prioritization = read("lib/blind-spots/a-tier-prioritization.ts");
...
const badge = read("components/blind-spots/EvidenceQualityBadge.tsx");
...
const reviewUi = read("components/blind-spots/BlindSpotReview.tsx");
...
const ranking = read("lib/blind-spots/blind-spot-ranking.ts");
if (!ranking.includes("blindSpotPrioritizationScore")) {
  fail("ranking must use blindSpotPrioritizationScore");
}

const dashboard = read("components/internal/ATierQualityDashboardPanel.tsx");
for (const label of [
  "A-tier rate",
  "B-tier rate",
  "C-tier rate",
...
} = await import("../lib/blind-spots/a-tier-prioritization.ts");
const {
...
if (!pkg.scripts?.["validate:a-tier-prioritization"]) {
  fail("package.json missing validate:a-tier-prioritization");
}

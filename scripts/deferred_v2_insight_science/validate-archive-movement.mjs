...
const required = [
  "packages/shared/types/archive-movement.ts",
  "packages/shared/lib/archive/archive-movement.ts",
  "packages/shared/lib/archive/archive-movement-copy.ts",
  "packages/shared/lib/metrics/archive-movement-events.ts",
  "apps/web/components/archive/ArchiveMovementCard.tsx",
  "apps/web/components/archive/LatestArchiveMovementCard.tsx",
  "apps/mobile/lib/features/archive_movement/archive_movement.dart",
  "apps/mobile/lib/widgets/archive_movement_card.dart",
];
...
const copy = fs.readFileSync(path.join(ROOT, "packages/shared/lib/archive/archive-movement-copy.ts"), "utf8");
const emotional = fs.readFileSync(
  path.join(ROOT, "packages/shared/lib/archive/archive-emotional-copy.ts"),
  "utf8",
);
for (const phrase of [
  "Archive update",
...
const card = fs.readFileSync(
  path.join(ROOT, "apps/web/components/archive/ArchiveMovementCard.tsx"),
  "utf8",
);
...
const events = fs.readFileSync(
  path.join(ROOT, "packages/shared/lib/metrics/archive-movement-events.ts"),
  "utf8",
);
if (!events.includes("voicememory_archive_updates")) fail("storage key missing");

const engine = fs.readFileSync(path.join(ROOT, "packages/shared/lib/archive/archive-movement.ts"), "utf8");
const priorities = [
  "confidence_changed",
  "evidence_increased",
  "status_changed",
...
const memoryPage = fs.readFileSync(path.join(ROOT, "apps/web/app/memory/page.tsx"), "utf8");
if (!memoryPage.includes("LatestArchiveMovementCard")) fail("memory page must show movement");

const recordDart = fs.readFileSync(
  path.join(ROOT, "apps/mobile/lib/screens/record_screen.dart"),
  "utf8",
);
...
const { clearArchiveMovementForEval, persistArchiveMovement, countArchiveMovementEvents } =
  await import("../../packages/shared/lib/metrics/archive-movement-events.ts");
const { buildArchiveMovementUpdate } = await import("../../packages/shared/lib/archive/archive-movement.ts");

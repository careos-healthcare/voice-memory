...
for (const rel of [
  "packages/shared/lib/archive/effort-compounds.ts",
  "packages/shared/lib/archive/effort-compounds-copy.ts",
  "packages/shared/lib/metrics/effort-compounds-events.ts",
  "apps/web/components/archive/EffortCompoundsPanel.tsx",
]) {
  if (!fs.existsSync(path.join(ROOT, rel))) fail(`missing ${rel}`);
}

const copy = fs.readFileSync(path.join(ROOT, "packages/shared/lib/archive/effort-compounds-copy.ts"), "utf8");
for (const phrase of [
  "harder to fool",
  "comparison point",
...
const panel = fs.readFileSync(path.join(ROOT, "apps/web/components/archive/EffortCompoundsPanel.tsx"), "utf8");

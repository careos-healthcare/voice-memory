...
const required = [
  "packages/shared/types/theory-curiosity-engine.ts",
  "packages/shared/lib/metrics/theory-curiosity-engine.ts",
  "packages/shared/lib/discover/theory-movement-feed.ts",
  "packages/shared/lib/discover/theory-movement-copy.ts",
  "apps/web/components/discover/TheoryMovementFeed.tsx",
  "apps/web/components/internal/TheoryCuriosityEnginePanel.tsx",
...
const discoverCopy = read("packages/shared/lib/theories/personal-theory-copy.ts");
if (!discoverCopy.includes("What your archive currently believes")) {
  fail("discover headline missing");
}

const movement = read("packages/shared/lib/discover/theory-movement-copy.ts");
for (const phrase of [
...
const engine = read("packages/shared/lib/metrics/theory-curiosity-engine.ts");
...
const feedUi = read("apps/web/components/discover/TheoryChangeFeed.tsx");
if (!feedUi.includes("TheoryMovementFeed")) {
  fail("TheoryChangeFeed must render TheoryMovementFeed");
}

const antiGamification = read("packages/shared/lib/metrics/theory-curiosity-engine.ts");
...
const { buildTheoryMovementFeed } = await import("../../packages/shared/lib/discover/theory-movement-feed.ts");
const { buildTheoryChangeFeed } = await import("../../packages/shared/lib/discover/theory-change-feed.ts");
const { buildTheoryResolutionFeed } = await import("../../packages/shared/lib/discover/theory-resolution-feed.ts");
...
const {
  ...
  isCuriousAnswer,
} = await import("../../packages/shared/lib/metrics/theory-curiosity-engine.ts");
const { trackTheoryEvent, THEORY_EVENTS, clearTheoryEventsForEval } = await import(
  "../../packages/shared/lib/theories/theory-events.ts"
);

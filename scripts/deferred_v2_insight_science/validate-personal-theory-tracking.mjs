...
const requiredFiles = [
  "packages/shared/types/personal-theory.ts",
  "packages/shared/lib/theories/personal-theory-status.ts",
  "packages/shared/lib/theories/personal-theory-map.ts",
  "packages/shared/lib/theories/theory-confidence-movement.ts",
  "packages/shared/lib/theories/personal-theory-copy.ts",
  "packages/shared/lib/metrics/theory-curiosity.ts",
  "apps/web/components/theories/EvidenceBuildingCard.tsx",
  "apps/web/components/theories/TheoryConfidenceMovement.tsx",
  "apps/web/components/theories/TheoryCuriosityPrompt.tsx",
  "apps/web/components/internal/TheoryCuriosityPanel.tsx",
];

for (const f of requiredFiles) mustExist(f);

const discover = read("packages/shared/lib/discover/discover-copy.ts");
const personalCopy = read("packages/shared/lib/theories/personal-theory-copy.ts");
...
for (const phrase of [
  ...
  if (!personalCopy.includes(phrase)) {
    const status = read("packages/shared/lib/theories/personal-theory-status.ts");
    if (phrase === "Theory under review" && status.includes(phrase)) continue;
    if (!read("apps/web/components/theories/EvidenceBuildingCard.tsx").includes(phrase.split(" ")[0])) {
      fail(`missing copy: ${phrase}`);
    }
  }
}

const movement = read("packages/shared/lib/theories/theory-confidence-movement.ts");
...
const curiosity = read("packages/shared/lib/metrics/theory-curiosity.ts");
...
const panel = read("apps/web/components/internal/TheoryCuriosityPanel.tsx");
if (!panel.includes("Theory Curiosity Rate")) {
  fail("internal Theory Curiosity Rate panel missing");
}

const evidenceCard = read("apps/web/components/theories/EvidenceBuildingCard.tsx");
...
for (const [file, needle] of [
  ["apps/web/app/discover/page.tsx", "EvidenceBuildingCard"],
  ["apps/web/app/blind-spots/page.tsx", "EvidenceBuildingCard"],
  ["apps/web/app/memory/page.tsx", "EvidenceBuildingCard"],
  ["apps/web/components/Recorder.tsx", "EvidenceBuildingCard"],
  ["apps/web/components/theories/TheoryCard.tsx", "TheoryConfidenceMovement"],
  ["apps/web/components/blind-spots/BlindSpotReview.tsx", "TheoryConfidenceMovement"],
]) {
...
const scanPaths = [
  "packages/shared/lib/theories/personal-theory-copy.ts",
  "packages/shared/lib/discover/discover-copy.ts",
  "apps/web/components/theories/EvidenceBuildingCard.tsx",
  "packages/shared/lib/metrics/theory-curiosity.ts",
];
...
} = await import("../../packages/shared/lib/theories/personal-theory-status.ts");
...
} = await import("../../packages/shared/lib/metrics/theory-curiosity.ts");
const { theoryToPersonalTheory } = await import("../../packages/shared/lib/theories/personal-theory-map.ts");

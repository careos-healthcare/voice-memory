...
} = await import("../lib/breakthrough/breakthrough-copy.ts");
const {
  insightProfileFromBlindSpotReview,
  buildBlindSpotAttribution,
} = await import("../lib/breakthrough/breakthrough-attribution.ts");
...
} = await import("../lib/breakthrough/breakthrough-prompt-gate.ts");
...
const panelPath = path.join(ROOT, "components/internal/BreakthroughTrackingPanel.tsx");
const pagePath = path.join(ROOT, "app/internal/blind-spot-discovery/page.tsx");
const reviewPath = path.join(ROOT, "components/blind-spots/BlindSpotReview.tsx");

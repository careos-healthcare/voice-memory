...
const modal = fs.readFileSync(path.join(ROOT, "components/auth/EmailCodeAuthModal.tsx"), "utf8");
if (!modal.includes("trackAuthVerified")) fail("EmailCodeAuthModal must track auth_verified");

const provider = fs.readFileSync(path.join(ROOT, "components/auth/AuthPromptProvider.tsx"), "utf8");
...
const { runAuthValueValidationTests } = await import(
  "../lib/reliability/auth-value-validation-tests.ts"
);

...
const required = [
  ...
  "components/product/ArchiveChangedMessage.tsx",
...
} = await import("../lib/product/archive-value-progress.ts");
...
} = await import("../lib/product/archive-value-metrics.ts");
...
const bannerSrc = fs.readFileSync(
  path.join(ROOT, "components/product/ArchiveValueBanner.tsx"),
  "utf8",
);
assert.ok(bannerSrc.includes("ctaLabel"), "banner must render snapshot CTA");

const progressSrc = fs.readFileSync(
  path.join(ROOT, "lib/product/archive-value-progress.ts"),
  "utf8",
);
...
const archiveValueCopySrc = fs.readFileSync(
  path.join(ROOT, "lib/product/archive-value-copy.ts"),
  "utf8",
);
assert.ok(
...
const headerSrc = fs.existsSync(path.join(ROOT, "components/SiteHeader.tsx"))
  ? fs.readFileSync(path.join(ROOT, "components/SiteHeader.tsx"), "utf8")
  : "";
...
for (const rel of ["lib/product/archive-value-copy.ts", "components/product/ArchiveValueBanner.tsx"]) {

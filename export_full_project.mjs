#!/usr/bin/env node
/**
 * Pack the full repository into exactly 3 plain-text export files:
 *   ~/Desktop/upload1/project_export_{1,2,3}.txt
 *
 * Usage: node export_full_project.mjs
 */
...
/** Directory names skipped entirely during walk (per export rules). */
...
/** Path fragments that exclude a directory or file. */
...
async function collectFiles(dir, relBase = "") {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      size = stat.size;
      if (size > 0 && (await looksBinary(full))) continue;
    } catch {
      continue;
    }
...
async function main() {
  ...
  console.log(
    `Target distribution (source bytes): ${bucketSizes.map((n) => n.toLocaleString()).join(", ")}`,
  );
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (const { rel, full } of buckets[i]) {
      ...
      if (contents == null) {
        parts.push(formatBlock(rel, `[SKIPPED: detected as binary at read time]\n`));
        continue;
      }
  ...
  const manifestPath = path.join(OUT_DIR, "export_manifest.txt");
  await fsp.writeFile(manifestPath, manifest, "utf8");

  console.log("");
  console.log("=== Export complete ===");
  console.log(`Output directory: ${OUT_DIR}`);
  for (const s of writtenStats) {
    console.log(`  ${path.basename(s.path)} — ${s.files} files, ${s.bytes.toLocaleString()} bytes`);
  }

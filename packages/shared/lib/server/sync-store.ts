...
function classifyBlobUpsert(
  existing: StoredSyncBlob | undefined,
  incoming: StoredSyncBlob,
): SyncBlobUpsertStatus {
  if (!existing) return "created";
  if (storedBlobsEqual(existing, incoming)) return "existing";
  return "updated";
}
...
export async function upsertEncryptedBlobs(
  ...
  const changedBlobs: StoredSyncBlob[] = [];

  for (const blob of blobs) {
    const status = classifyBlobUpsert(byId.get(blob.id), blob);
    statusMatrix.push({ id: blob.id, type: blob.type, status });
    if (status === "existing") continue;
    byId.set(blob.id, blob);
    changedBlobs.push(blob);
  }

  if (changedBlobs.length > 0) {
    appendChangeRecords(store, changedBlobs);
  ...
  return {
    manifest: manifestFromStore(store),
    statusMatrix,
  };
}

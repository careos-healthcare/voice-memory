...
import type {
  ...
  SyncBlobUpsertStatus,
  SyncChangeRecord,
  SyncChangesResponse,
  SyncManifest,
  SyncPushUpsertReport,
} from "@/types/sync";
...
function encryptedPayloadsEqual(
  left: EncryptedPayload,
  right: EncryptedPayload,
): boolean {
  return (
...
function storedBlobsEqual(left: StoredSyncBlob, right: StoredSyncBlob): boolean {
  return (
    left.type === right.type &&
    left.updatedAt === right.updatedAt &&
    left.byteLength === right.byteLength &&
    encryptedPayloadsEqual(left.encrypted, right.encrypted)
  );
}

async function readStoredSyncBlob(
  userId: string,
  blob: StoredSyncBlob,
): Promise<StoredSyncBlob | null> {
  const result = await dbQuery<{
    ...
    `SELECT blob_id, blob_type, updated_at, encrypted_payload
     FROM sync_blobs
     WHERE user_id = $1 AND blob_type = $2 AND blob_id = $3`,
    [userId, blob.type, blob.id],
  );
...
async function writeStoredSyncBlobChange(
  userId: string,
  blob: StoredSyncBlob,
): Promise<void> {
  await dbQuery(
    ...
    [userId, blob.type, blob.id, JSON.stringify(blob.encrypted), blob.updatedAt],
  );
...
): Promise<SyncPushUpsertReport> {
  const statusMatrix: SyncBlobStatusMatrixEntry[] = [];

  for (const blob of blobs) {
    rejectPlaintextBlobFields(blob as unknown as Record<string, unknown>);
    assertEncryptedPayloadOnly(blob.encrypted);

    const existing = await readStoredSyncBlob(userId, blob);
    let status: SyncBlobUpsertStatus;
    if (!existing) {
      status = "created";
    } else if (storedBlobsEqual(existing, blob)) {
      status = "existing";
    } else {
      status = "updated";
    }

    statusMatrix.push({ id: blob.id, type: blob.type, status });
    if (status === "existing") continue;
    await writeStoredSyncBlobChange(userId, blob);
  }

  return {
    manifest: await readSyncManifestPostgres(userId),
    statusMatrix,
  };
}

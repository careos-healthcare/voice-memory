...
const WS_ROUTE_PATH = path.join(process.cwd(), "apps/api/app/api/live-audio/ws/route.ts");
const WS_UPGRADE_PATH = path.join(process.cwd(), "packages/shared/lib/live-audio/ws-upgrade.ts");
const SERVER_ENTRY_PATH = path.join(process.cwd(), "apps/api/server.entry.ts");
const SERVER_BUNDLE_PATH = path.join(process.cwd(), "apps/api/dist/main.js");

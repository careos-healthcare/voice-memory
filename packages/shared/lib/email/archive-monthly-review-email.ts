...
function pickStrongestChange(review: ArchiveMonthlyReview): string | null {
  return review.whatChanged[0]?.statement?.trim() || null;
}
...
export function buildArchiveMonthlyReviewSubject(
  ...
  if (review.whatChanged[0]?.statement?.trim()) {
...
export function buildArchiveMonthlyReviewBody(
  ...
  const strongest = pickStrongestChange(review);
  ...
  if (strongest) {
    lines.push("Biggest change", strongest, "");
  }
  if (change && change !== strongest) {
    lines.push("What shifted", change, "");
  }
  if (surprise) {

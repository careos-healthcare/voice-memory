...
/** Smoke journey only — excludes purchase/restore (used when livePurchases=false). */
const TRACK_SMOKE_BOOLEAN_FIELDS: (keyof Omit<StoreTrackEvidence, "timestamp">)[] = [
  "success",
  "build_uploaded",
  "build_installed",
  "onboarding_completed",
...
function trackMissingRequirements(
  ...
  relPath: string,
  fields: (keyof Omit<StoreTrackEvidence, "timestamp">)[] = TRACK_BOOLEAN_FIELDS,
): string[] {
  if (!evidence) {
...
export function testflightSmokeMissingRequirements(
  evidence: StoreTrackEvidence | null = readTestflightEvidence(),
): string[] {
  return trackMissingRequirements(
    evidence,
    "mobile/evidence/testflight_tested.json",
    TRACK_SMOKE_BOOLEAN_FIELDS,
  );
}
...
export function isTestflightSmokePassing(
  evidence: StoreTrackEvidence | null = readTestflightEvidence(),
): boolean {
  return testflightSmokeMissingRequirements(evidence).length === 0;
}

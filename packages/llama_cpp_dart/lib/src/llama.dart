...
class Llama {
  ...
  /// Cooperative abort flag checked between decode steps and via native callback.
  static bool generationAbortRequested = false;

  static void requestGenerationAbort() {
    generationAbortRequested = true;
  }

  static void clearGenerationAbort() {
    generationAbortRequested = false;
  }

  static bool _generationAbortCallback(Pointer<Void> data) {
    return generationAbortRequested;
  }

  static final _generationAbortCallbackPointer =
      Pointer.fromFunction<Bool Function(Pointer<Void>)>(
    _generationAbortCallback,
    true,
  );

  void _installAbortCallback(Pointer<llama_context> ctx) {
    lib.llama_set_abort_callback(
      ctx,
      _generationAbortCallbackPointer,
      nullptr,
    );
  }
  ...
  void _initializeLlama(
    ...
    try {
      ...
      _installAbortCallback(loadedContext);
    } catch (e) {

...
function flutterRoot(): string {
  return path.join(repoRoot(), "apps", "voicememory_mobile");
}
...
export function collectStructuralEvidenceSignals(): StructuralEvidenceSignal[] {
  ...
  const pubspec = readText("apps/voicememory_mobile/pubspec.yaml");
  const iosChecklist = readText("apps/voicememory_mobile/docs/IOS_RELEASE_CHECKLIST.md");
  const androidGradle = readText("apps/voicememory_mobile/android/app/build.gradle.kts");
  ...
  signals.push({
    ...
    passed: false,
    note:
      readText("apps/voicememory_mobile/lib/storage/journal_store.dart").includes("class JournalStore")
  ...
  signals.push({
    ...
    passed: readText("apps/voicememory_mobile/lib/services/sync_service.dart").includes("syncNow"),
  ...
  const rcIntegrated =
    ...
    readText("apps/voicememory_mobile/lib/billing/revenuecat_service.dart").includes(
      "RevenueCatService",
    );
  ...
  signals.push({
    ...
    passed:
      rcIntegrated &&
      readText("apps/voicememory_mobile/lib/screens/restore_purchases_screen.dart").includes(
        "restoreNative",
      ),
    note: rcIntegrated
  ...
  signals.push({
    ...
    passed: !readText("apps/voicememory_mobile/lib/screens/pricing_screen.dart").includes(
      "launchUrl",
    ),
    note: rcIntegrated

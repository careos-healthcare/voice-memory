...
function socketMessage(socket: WebSocket): Promise<Record<string, unknown>> {
...
export async function pair(invitation: PairingInvitation): Promise<void> {
  ...
  socket.close();
  if (response.type !== "paired") throw new Error(String(response.message));
...
async function identity(): Promise<BridgeIdentity> {
  const values = await api.storage.local.get([identityStorageKey]);
  const value = values[identityStorageKey];
  if (!value || typeof value !== "object") throw new Error("Browser is not paired");
  return value as BridgeIdentity;
}

export async function clip(payload: ClipPayload): Promise<Record<string, unknown>> {
  ...
  const signed = new TextEncoder().encode(
    `${stored.extensionId}|${timestamp}|${nonceText}`,
  );
  ...
  if (ready.type !== "ready") throw new Error(String(ready.message));

  const sessionKey = await crypto.subtle.importKey(
    "raw",
    decode(stored.sessionKey),
    "AES-GCM",
    false,
    ["encrypt"],
  );
    ...
    await crypto.subtle.encrypt(
      {
        ...
        additionalData: new TextEncoder().encode(
          `browser-clip-v1|${stored.extensionId}`,
        ),
        tagLength: 128,
      },
      sessionKey,
  ...
  socket.close();
  if (result.type !== "stored") throw new Error(String(result.message));
  return result;
}

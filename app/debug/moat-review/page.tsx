"use client";

import { useEffect, useState } from "react";
import Link from "next/link";
import { RefreshCw } from "lucide-react";

import { MoatReviewScoreboard } from "@/components/debug/MoatReviewScoreboard";
import { SiteHeader } from "@/components/SiteHeader";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { buildMoatReviewReport } from "@/lib/observation/moat-review";
import type { MoatReviewReport } from "@/types/observation-workflow";

export default function MoatReviewPage() {
  const [report, setReport] = useState<MoatReviewReport | null>(null);

  const refresh = () => {
    setReport(buildMoatReviewReport());
  };

  useEffect(() => {
    refresh();
  }, []);

  return (
    <div className="min-h-screen bg-zinc-950">
      <div className="mx-auto max-w-4xl px-4 pb-20 sm:px-6">
        <SiteHeader />

        <header className="mt-2 flex items-start justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-violet-300/80">Debug only</p>
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-white">Moat review</h1>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-zinc-400">
              Single scoreboard for exact behavior vs pre-monetization targets.
            </p>
          </div>
          <Button type="button" variant="ghost" size="sm" onClick={refresh}>
            <RefreshCw className="h-4 w-4" />
            Refresh
          </Button>
        </header>

        {!report ? (
          <Card className="mt-6">
            <CardContent className="py-12 text-center text-sm text-zinc-500">Loading…</CardContent>
          </Card>
        ) : !report.hasData ? (
          <Card className="mt-6">
            <CardContent className="py-12 text-center text-sm text-zinc-500">
              Add reflections and revisit older entries to populate the scoreboard.
            </CardContent>
          </Card>
        ) : (
          <div className="mt-6">
            <MoatReviewScoreboard report={report} />
          </div>
        )}

        <div className="mt-10 flex flex-wrap gap-3 text-sm">
          <Link href="/debug/moat" className="text-violet-300 hover:text-violet-200">
            Full moat metrics →
          </Link>
          <Link href="/debug/monetization-readiness" className="text-violet-300 hover:text-violet-200">
            Monetization gate →
          </Link>
          <Link href="/debug/retention-study" className="text-zinc-500 hover:text-zinc-300">
            Retention study →
          </Link>
        </div>
      </div>
    </div>
  );
}

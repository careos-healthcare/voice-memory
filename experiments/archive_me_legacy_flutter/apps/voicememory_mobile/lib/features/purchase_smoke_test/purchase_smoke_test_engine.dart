...
abstract final class PurchaseSmokeTestEngine {
  ...
  static Future<PurchaseSmokeTestSnapshot> build() async {
    ...
    try {
      final diagnostics = await repository.loadDiagnostics();
      final offers = await repository.loadOffers();
      final entitlements = await repository.refresh();
      final storeAvailable =
          diagnostics.availability == SubscriptionAvailability.available;

      return buildFromInput(
        PurchaseSmokeTestInput(
          billingConfigured: storeAvailable,
          revenueCatInitialized: storeAvailable,
          offeringsLoaded: diagnostics.offersLoaded,
          currentOfferingExists: offers.isNotEmpty,
          hasMonthlyPackage: offers.any(
            (offer) => offer.period == SubscriptionPeriod.monthly,
          ),
          hasAnnualPackage: offers.any(
            (offer) => offer.period == SubscriptionPeriod.annual,
          ),
          entitlementReadable: true,
          isPro: entitlements.isPro,
          lastErrorSafe: sanitizeDebugText(diagnostics.lastError),
        ),
      );
    } on Object catch (error) {
      return buildFromInput(
        PurchaseSmokeTestInput(
          lastErrorSafe: sanitizeDebugText(error.runtimeType.toString()),
        ),
      );
    }
  }

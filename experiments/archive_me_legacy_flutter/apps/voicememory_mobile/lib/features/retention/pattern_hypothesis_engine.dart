...
class PatternHypothesisEngine {
  ...
  List<String> _evidenceLines(
    ...
    if (selected != null) {
      for (final c in selected.evidenceChips) {
        final quote = _exactQuote(c, eligible);
        if (quote == null) continue;
        out.add('Exact saved wording: “$quote”.');
    ...
    for (final chip in chips) {
      final quote = _exactQuote(chip, eligible);
      if (quote == null) continue;
      final line = 'Exact saved wording: “$quote”.';
  ...
  String? _exactQuote(String candidate, List<JournalEntry> entries) {
    final needle = candidate.trim();
    if (needle.length < 4) return null;
    for (final entry in entries.reversed) {
      final start = entry.transcript.toLowerCase().indexOf(
        needle.toLowerCase(),
      );
      if (start >= 0) {
        return entry.transcript.substring(start, start + needle.length);
      }
    }
    return null;
  }

...
class DiscoverYourselfEngine {
  ...
  DiscoverArchiveAnswer _answerRepeating(List<JournalEntry> entries) {
    ...
    if (themes.isEmpty) {
      return DiscoverArchiveAnswer(
        ...
        answerLines: const [
          'Themes are still emerging — save a few more moments.',
        ],
        citedEntryIds: const [],
      );
    }
    ...
    return DiscoverArchiveAnswer(
      prompt: 'What do I keep repeating?',
      answerLines: [
        '“${ids.name}” appears in ${top.first.value} saved moments — the most repeated theme so far.',
      ],
  ...
  DiscoverArchiveAnswer _answerWhatMatters(
    ...
    return DiscoverArchiveAnswer(
      prompt: 'What matters most to me?',
      answerLines: const ['Save a few more moments so patterns can form.'],
      citedEntryIds: const [],
    );
  }

...
class DiscoverLocalEngine {
  ...
  static DiscoverLocalFeed build({
    ...
    for (final entry in current.entries) {
      ...
      if (before == 0 && after >= 2) {
        newItems.add(
          DiscoverChangeItem(
            title: entry.key,
            detail: 'May be emerging across recent moments.',
            kind: 'new',
          ),
        );
        ...
        weakened.add(
          DiscoverChangeItem(
            title: entry.key,
            detail: 'May be appearing less in recent moments.',
            kind: 'weakened',
          ),
        );
      }
    ...
    if (entries.length > (baselineThemes['__count'] ?? 0)) {
      evidence.add(
        DiscoverChangeItem(
          title: 'New saved moments',
          detail:
              '${entries.length - (baselineThemes['__count'] ?? 0)} new saved moment(s) since last visit.',
          kind: 'evidence',
        ),
      );
    }

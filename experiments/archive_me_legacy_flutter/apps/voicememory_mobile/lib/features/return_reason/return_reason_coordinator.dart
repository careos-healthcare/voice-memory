...
import 'return_reason_engine.dart';
import 'return_reason_models.dart';
import 'return_reason_store.dart';
...
class ReturnReasonCoordinator {
  ...
  static const discoverTabIndex = beliefsTabIndex;
  ...
  static Future<void> onLeftBeliefsTab() async {
    try {
      ...
      ProductAnalytics.trackStrings('return_reason_generated', {
        'kind': card.kind.name,
      });
    } catch (e) {
      assert(() {
  ...
  static ReturnReasonCard? _cardFromState(ReturnReasonState saved) {
    ...
    if (kind == ReturnReasonKind.conflictingEvidence) {
      return ReturnReasonCard(
        ...
        bodyLines: const [
          'Your moments pull in different directions.',
    ...
    if (saved.unresolvedPatterns.length >= 2) {
      ...
      return ReturnReasonCard(
        ...
        bodyLines: [
          ...
          '$needed more moment${needed == 1 ? '' : 's'} may reveal a stronger pattern.',
        ],

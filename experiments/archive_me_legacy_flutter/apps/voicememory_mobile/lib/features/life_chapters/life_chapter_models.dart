...
class LifeChapter {
  ...
  AiExplainability get explainability => AiExplainability(
    confidence: (50 + evidenceIds.length * 10).clamp(0, 90),
    evidence: importantQuotes.isEmpty
        ? [AiEvidenceSource(sourceId: id, excerpt: themeSummary)]
        : importantQuotes
              .map(
                (quote) => AiEvidenceSource(
                  sourceId: quote.entryId,
                  excerpt: quote.quote,
                ),
              )
              .toList(),
    reasoning: [
      'Recordings were ordered by date.',
      'Recurring themes were grouped within this period.',
      'The chapter was retained only after meeting the evidence threshold.',
    ],
    alternativeExplanation:
        'These recordings may represent several overlapping periods rather '
        'than one distinct chapter.',
    uncertainty:
        'Recordings outside ArchiveMe and gaps between entries are not visible.',
  );

...
abstract class FirstSessionCoordinator {
  ...
  static Future<FirstSessionPattern?> buildFromEntry(
    ...
    final pattern = _engine.build(
      entry,
      alternativeIndex: alternativeIndex,
      preferredCategoryBoosts: boosts,
    );
    final evidence = <TranscriptEvidenceCitation>[];
    for (final phrase in pattern.matchedPhrases) {
      final start = entry.transcript.indexOf(phrase);
      if (start < 0) continue;
      evidence.add(
    ...
    final transcripts = {entry.id: entry.transcript};
    final gated = ExplainableConclusionMappers.fromFirstSessionPattern(
      source: pattern,
    ...
    if (AppServices.isInitialized) {
      await AppServices.instance.explainabilityHistoryStore.appendIfAbsent(
        gated,
      );
    }
    return pattern;

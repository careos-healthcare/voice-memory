...
abstract class ArchiveSynthesisPackBuilder {
  ...
  static const int packVersion = 3;
  ...
  static Map<String, dynamic> build({
    ...
    final primaryTheory = _theoryRef(
      ranking?.primaryTheory ?? _theoryAsRanked(theory),
    );
    final secondaryTheories = (ranking?.secondaryTheories ?? const [])
        .map(_theoryRef)
        .whereType<Map<String, dynamic>>()
        .toList();
    final lifecycle = _lifecycleJson(view.lifecycle);
    final changeFeed = _changeFeedJson(view.changeFeed);
    final contradictions = view.contradictions
        .map(
          (c) => {
            'id': c.id,
            'youSay': c.youSay,
        ...
        .toList();
    final blindSpots = view.blindSpots
        .map(
          (b) => {
            'id': b.id,
            'headline': b.headline,
        ...
        .toList();
    final surprises = view.surprises.observations
        .map(
          (s) => {
            'id': s.id,
    ...
    final pack = <String, dynamic>{
      ...
      'secondaryTheories': secondaryTheories,
      'theory': theory == null
          ? null
          : {
      ...
      'slices': {
        'theory': {
          'primary': primaryTheory,
          'secondary': secondaryTheories,
          'lifecycle': lifecycle,
        },
        'change': changeFeed,
        'patterns': {
  ...
  static Map<String, dynamic> _lifecycleJson(BeliefLifecycleView lifecycle) {
    BeliefLifecycleEntry? current = lifecycle.current;
    return {
  ...
  static Map<String, dynamic> _reflectionRow(JournalEntry entry) {
    ...
    return {
      ...
      // Deliberately unmodified: backend citation offsets are verified against
      // this exact UTF-16 source string.
      'canonicalTranscript': entry.transcript,
      'mood': r.mood,

...
import '../domain/models/vault_file_metadata.dart';
import 'emergency_vault_storage.dart';
import 'local_audio_vault_reader.dart';
...
typedef EmergencyChunkQueuedCallback = Future<void> Function();
...
class LocalAudioVault {
  LocalAudioVault({
    ...
    this._emergencyChunkStorage,
    this._onEmergencyChunkQueued,
  ...
  final EmergencyVaultStorage? _emergencyChunkStorage;
  final EmergencyChunkQueuedCallback? _onEmergencyChunkQueued;
  ...
  Future<void> _ensureKey() async {
    _secretKey ??= await _vaultKeyProvider.getOrCreateMasterKey();
  }
  ...
  Future<void> _appendEncryptedFrame(Uint8List pcmBytes) async {
    ...
    try {
      final encryptedRecord = await _cipher.encryptVaultFrameRecord(
        rawBytes: pcmBytes,
        secretKey: secretKey,
      );
      await sink.writeFrom(encryptedRecord);
      _frameCount++;
      await _enqueueEmergencyChunk(encryptedRecord);
    } catch (error) {
      LiveAudioPipelineLog.failure('offline_vault_append', error);
    }
  }

  Future<void> _enqueueEmergencyChunk(List<int> encryptedRecordBytes) async {
    final storage = _emergencyChunkStorage;
    final sessionId = _activeSessionId;
    if (storage == null || sessionId == null || encryptedRecordBytes.isEmpty) {
      return;
    }
    ...
    try {
      await storage.enqueueChunk(
        sessionId: sessionId,
        bytes: encryptedRecordBytes,
      );
      final trigger = _onEmergencyChunkQueued;
      if (trigger != null) {
        unawaited(trigger());
      }
    } catch (error) {
      LiveAudioPipelineLog.failure('emergency_chunk_enqueue', error);
    }
  }

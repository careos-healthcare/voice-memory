...
import '../../cognitive_telemetry/application/live_voice_acoustic_session_bridge.dart';
import '../../cognitive_telemetry/application/live_voice_pitch_contour_collector.dart';
...
import '../presentation/controllers/live_voice_playback_player.dart';
...
class LiveVoiceCaptureService
    implements Listenable, LiveVoiceTelemetrySource, LiveVoiceCaptureLifecycle {
  LiveVoiceCaptureService({
    ...
    this._acousticSessionBridge,
    LiveVoicePitchContourCollector? pitchContourCollector,
    LiveVoicePlaybackPlayer? playbackPlayer,
    LiveVoicePitchContourCollector? playbackPitchContourCollector,
    this.useIsolateAudioPipeline = false,
    this.maxReconnectAttempts = 1,
    RetryPolicy? reconnectRetryPolicy,
  }) : reconnectRetryPolicy =
           reconnectRetryPolicy ??
           RetryPolicy(maxAttempts: maxReconnectAttempts),
       ...
       _pitchContourCollector =
           pitchContourCollector ?? LiveVoicePitchContourCollector(),
       _playbackPlayer = playbackPlayer ?? LiveVoicePlaybackPlayer(),
       _playbackPitchContourCollector =
           playbackPitchContourCollector ?? LiveVoicePitchContourCollector() {
    _controller.addListener(_emitDiagnostics);
    _playbackQueueUiSubscription = _playback.queueDepthStream.listen(
      _syncPlaybackUiFromQueue,
    );
  }
  ...
  final LiveVoiceAcousticSessionBridge? _acousticSessionBridge;
  final LiveVoicePitchContourCollector _pitchContourCollector;
  final LiveVoicePlaybackPlayer _playbackPlayer;
  final LiveVoicePitchContourCollector _playbackPitchContourCollector;
  final bool useIsolateAudioPipeline;
  final int maxReconnectAttempts;
  final RetryPolicy reconnectRetryPolicy;
  final _durationController = StreamController<int>.broadcast();
  final _audioDecibelController = StreamController<double>.broadcast();
  ...
  StreamSubscription<int>? _playbackQueueUiSubscription;
  int _hardwareSampleRate = liveInputSampleRateHz;
  Duration _queuedPlaybackDuration = Duration.zero;
  ...
  LiveVoicePlaybackPlayer get playbackPlayer => _playbackPlayer;
  List<double> get playbackPitchContour =>
      _playbackPitchContourCollector.snapshot();
  @override
  ...
  Future<void> start({
    ...
    try {
      _resetDiagnostics();
      _pitchContourCollector.reset();
      _playbackPitchContourCollector.reset();
      _transcripts.reset();
      _lastAudioLevelAt = null;
      if (!_audioDecibelController.isClosed) {
        _audioDecibelController.add(-60);
      }
  ...
  void _sampleInputDecibels(List<int> pcm16LeBytes) {
    final now = DateTime.now();
    final lastSample = _lastAudioLevelAt;
    if (lastSample != null &&
        now.difference(lastSample) < const Duration(milliseconds: 50)) {
      return;
    }
    _lastAudioLevelAt = now;

    var sumSquares = 0.0;
    var sampleCount = 0;
    // A sparse RMS sample keeps this callback inexpensive even for large
    // hardware buffers; visual interpolation fills the gaps at display rate.
    for (var offset = 0; offset + 1 < pcm16LeBytes.length; offset += 16) {
      final low = pcm16LeBytes[offset] & 0xff;
      final high = pcm16LeBytes[offset + 1] & 0xff;
    ...
    if (sampleCount == 0 || _audioDecibelController.isClosed) return;
    final rms = math.sqrt(sumSquares / sampleCount);
    final decibels = rms <= 0.001
        ? -60.0
        : (20 * math.log(rms) / math.ln10).clamp(-60.0, 0.0);
  ...
  void _recordPitchSample(Int16List frame) {
    _pitchContourCollector.ingestPcm16Frame(
      frame,
  ...
  Future<void> _flushPlaybackOutput() async {
    await _playback.flush();
    _resetPlaybackUi();
  }

  Future<void> _stopPlaybackOutput() async {
    await _playback.stop();
    _resetPlaybackUi();
  }

  void _trackOutputChunk(List<int> pcmBytes) {
    _queuedPlaybackDuration += LiveVoicePlaybackPlayer.durationForPcm16LeBytes(
      pcmBytes,
      sampleRateHz: liveOutputSampleRateHz,
      numChannels: liveOutputNumChannels,
    );
    _playbackPitchContourCollector.ingestPcm16LeBytes(
      pcmBytes,
      sampleRateHz: liveOutputSampleRateHz,
    );
    _notifyListeners();
  ...
  void _syncPlaybackUiFromQueue(int depth) {
    if (depth <= 0) {
      _resetPlaybackUi();
      return;
    }

    final total = _queuedPlaybackDuration <= Duration.zero
        ? const Duration(milliseconds: 250)
        : _queuedPlaybackDuration;

    if (_playbackPlayer.state == PlaybackState.stopped) {
      _playbackPlayer.loadSession(totalDuration: total);
      _playbackPlayer.play();
      return;
    }

    _playbackPlayer.updateTotalDuration(total);
    if (!_playbackPlayer.isPlaying) {
      _playbackPlayer.play();
    }
  }

  void _resetPlaybackUi() {
    _queuedPlaybackDuration = Duration.zero;
    _playbackPitchContourCollector.reset();
  ...
  Future<CapturePipelineResult> stopAndSave({
    ...
    await _stopPlaybackOutput();
    final vaultedFrames = _localVault.isActive ? _localVault.frameCount : 0;
    final sessionId = _currentSessionId;
    final pitchContour = _pitchContourCollector.snapshot();
    await _deactivateOfflineVault();
    _acousticSessionBridge?.stageAfterVaultCommit(
      sessionId: sessionId,
  ...
  void _ingestNonFaultEvent(LiveServerEvent event) {
    if (event is LiveInterruptedEvent) {
      ...
      unawaited(_flushPlaybackOutput());
      return;
    }
    ...
    if (event is LiveAudioOutputEvent) {
      ...
      _trackOutputChunk(event.pcmBytes);
  ...
  Future<void> _handleSessionFault(String reason) async {
    ...
    LiveAudioPipelineLog.sessionFault(
      ...
      recoverable: reconnectRetryPolicy.shouldRetry(_reconnectAttempts),
    );

    if (reconnectRetryPolicy.shouldRetry(_reconnectAttempts)) {
      _reconnectAttempts++;
      final delay = reconnectRetryPolicy.calculateDelay(_reconnectAttempts);
  ...
  Future<void> dispose() async {
    ...
    _controller.clearPcmCaptureHandler();
    await cancel();
    await _playbackQueueUiSubscription?.cancel();
    _playbackQueueUiSubscription = null;
    await _serverEventsSubscription?.cancel();
    _serverEventsSubscription = null;
    await _playback.dispose();
    _playbackPlayer.dispose();
    await _durationController.close();
    await _audioDecibelController.close();

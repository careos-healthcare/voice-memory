...
class LocalCheckInReminderBackend
    implements CheckInReminderBackend, QuietReminderBackend {
  ...
  static const String quietChannelId = 'archive_quiet_returns';
  static const String quietChannelName = 'Quiet archive returns';
  static const String quietChannelDescription =
      'Low-priority reminders for active watch targets.';
    ...
    try {
      ...
      if (!kIsWeb && defaultTargetPlatform == TargetPlatform.iOS) {
        _backgroundChannel.setMethodCallHandler((call) async {
          if (call.method != 'reschedulePendingReminders') {
      ...
      await android0?.createNotificationChannel(
        const AndroidNotificationChannel(
          quietChannelId,
          quietChannelName,
          description: quietChannelDescription,
  ...
  Future<void> scheduleQuiet({
    ...
    try {
      await _plugin.zonedSchedule(
        ...
        notificationDetails: const NotificationDetails(
          android: AndroidNotificationDetails(
            quietChannelId,
            quietChannelName,
            channelDescription: quietChannelDescription,
            importance: Importance.low,
      ...
      // Quiet scheduling failures must never crash the archive loop.
    }
  }

...
import '../notifications/thread_return_notification_service.dart';
...
abstract class WatchForCoordinator {
  ...
  static Future<WatchForAcceptance> acceptSuggestedWatchFor(
    WatchForItem item, {
    ArchiveEntitlementReader? entitlementReader,
  }) async {
    if (ScreenshotMode.enabled) {
      return WatchForAcceptance.accepted(item);
    }
    final reader =
        entitlementReader ?? ArchiveEntitlementReader.forAccessCheck();
    final isPro = await reader.isPro;
    final store = _store();
    final result = await store.addActive(item, isPro: isPro);
    if (result == WatchForAddResult.requiresPro) {
      return WatchForAcceptance.requiresPro(item);
    }
    await ThreadReturnNotificationService.onWatchTargetActivated(item);
    return WatchForAcceptance.accepted(item);
  }
  ...
  static Future<void> skipPendingForToday({DateTime? now}) async {
    ...
    await ThreadReturnNotificationService.threadCompleted(pending.id);
    await store.removeActive(pending.id);
  ...
  static Future<void> snoozePendingForSevenDays({DateTime? now}) async {
    if (ScreenshotMode.enabled) return;
    final clock = now ?? DateTime.now();
    final store = _store();
    final pending = await store.readPending();
    if (pending == null) return;
    await ThreadReturnNotificationService.snoozeThread(
      pending.sourceReflectionId,
      now: clock,
    );
    await store.updateActive(
      pending.copyWith(targetDate: clock.add(const Duration(days: 7))),
    );
  }
...
class WatchForAcceptance {
  const WatchForAcceptance._({required this.item, required this.requiresPro});

  factory WatchForAcceptance.accepted(WatchForItem item) {
    return WatchForAcceptance._(item: item, requiresPro: false);
  }

  factory WatchForAcceptance.requiresPro(WatchForItem item) {
    return WatchForAcceptance._(item: item, requiresPro: true);
  }

  final WatchForItem item;

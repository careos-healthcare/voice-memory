...
import '../archive_deep_dive/archive_deep_dive_engine.dart';
...
import '../belief_evolution/belief_evolution_service.dart';
import 'archive_analyst_belief_catalog.dart';
import 'archive_analyst_confidence_engine.dart';
import 'topical_counter_evidence.dart';
import 'archive_analyst_gate.dart';
...
class ArchiveAnalystEngine {
  ...
  Future<ArchiveAnalystReport> build({
    ...
    final v1 = await intelligenceEngine.buildArchiveView(
      entries: entries,
      state: state,
  ...
  /// Creates an evidence-bound archive digest suitable for text-to-speech.
  String buildAudioDigest(ArchiveAnalystReport report) {
    if (!report.hasReport) return '';

    final summary = report.evidenceSummary;
    final sentences = <String>[
      'Archive Audio Digest.',
      'This review considered ${summary.eligibleReflectionCount} saved moments '
          'across ${summary.dateSpanLabel}.',
    ];

    if (report.currentBeliefs.isNotEmpty) {
      var primary = report.currentBeliefs.first;
      for (final belief in report.currentBeliefs) {
        if (belief.isPrimary) {
          primary = belief;
          break;
        }
      ...
      sentences.add(
        'The primary belief your archive is tracking is: '
        '${primary.statement}. It has ${primary.evidenceCount} supporting '
        'moments and ${primary.counterEvidenceCount} counter examples.',
      );
    }

    if (report.emergingBeliefs.isNotEmpty) {
      sentences.add(
        'An emerging belief is: ${report.emergingBeliefs.first.statement}.',
      );
    }
    if (report.fadingBeliefs.isNotEmpty) {
      sentences.add(
        'A belief appearing less often is: '
        '${report.fadingBeliefs.first.statement}.',
      );
    }

    if (summary.contradictionCount > 0) {
      sentences.add(
        'The archive found ${summary.contradictionCount} '
        '${summary.contradictionCount == 1 ? 'contradiction' : 'contradictions'} '
        'worth reviewing.',
      );
    }

    sentences.add(
      'This digest is a summary of your own evidence, not a diagnosis.',
    );

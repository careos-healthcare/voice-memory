...
class ArchiveBeliefsPresenter {
    ...
    if (belief != null && !cards.any((c) => c.statement == belief.statement)) {
      cards.add(
        ArchiveBeliefCardModel(
          ...
          evidenceSummary:
              'Appeared in ${belief.evidenceCount} saved moment${belief.evidenceCount == 1 ? '' : 's'}.',
          whyExplanation:
              'Named from recurring themes across your saved moments.',
    ...
    for (final spot in archiveV1?.blindSpots ?? const []) {
      cards.add(
        ArchiveBeliefCardModel(
          ...
          evidenceSummary:
              'Observed across ${spot.evidenceCount} saved moment${spot.evidenceCount == 1 ? '' : 's'}.',
  ...
  static ArchiveBeliefCardModel _fromRanked(
    ...
    return ArchiveBeliefCardModel(
      ...
      evidenceSummary:
          'Appeared in ${t.evidenceCount} saved moment${t.evidenceCount == 1 ? '' : 's'}.',
      whyExplanation:
          'ArchiveMe ranked this from recurring themes in your saved moments.',
      section: section,
      timeline: _timelineFromEntries(t.supportingEntries),
      conclusion: 'This pattern has enough moments to show up clearly.',
    );
  }
  ...
  static ArchiveBeliefCardModel _fromLifecycle(
    ...
    final why = switch (entry.status) {
      BeliefLifecycleStatus.emerging =>
        'First detected recently across your saved moments.',
      BeliefLifecycleStatus.stable =>
        'Has remained steady across your saved moments over time.',
      BeliefLifecycleStatus.weakening =>
        'Shows up less often in recent moments.',
      BeliefLifecycleStatus.dormant =>
        'Mentioned less often in recent recordings.',
      BeliefLifecycleStatus.noLongerDetected =>
        'No longer appears in recent moments.',
    };
    return ArchiveBeliefCardModel(
      ...
      evidenceSummary:
          'Tracked since ${_monthYear(entry.firstSeen)} in your saved moments.',
      whyExplanation: why,
      section: section,
  ...
  static ArchiveBeliefCardModel _fromDiscover(
    ...
    return ArchiveBeliefCardModel(
      ...
      evidenceSummary: 'Pattern shift detected in recent moments.',
    ...
    for (final e in sorted.take(2)) {
      ...
      cards.add(
        ArchiveBeliefCardModel(
          ...
          whyExplanation:
              'ArchiveMe noticed this topic repeating across months of saved moments.',
          section: ArchiveBeliefSection.hiddenPattern,
        ),
      );
    }

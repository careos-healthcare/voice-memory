...
class _DeveloperDiagnosticsScreenState
  ...
  Future<void> _refresh() async {
    ...
    try {
      final h = await AppServices.instance.journalSyncApi.health();
      ...
      final cachedState = await repository.loadCachedState();
      final appLockEnabled = await AppLockService.instance.isEnabled();
      final subscriptionDiagnostics = await repository.loadDiagnostics();
      final currentState = repository.currentState ?? cachedState;
        ...
        setState(() {
          ...
          final proAccessSignals = ProAccessEnforcementLocalSignals(
            revenueCatConfigured:
                subscriptionDiagnostics.availability ==
                SubscriptionAvailability.available,
            revenueCatApiKeyMissing:
                subscriptionDiagnostics.availability ==
                SubscriptionAvailability.notConfigured,
            productsLoaded:
                subscriptionDiagnostics.offersLoaded &&
                subscriptionDiagnostics.offerCount > 0,
            proStateReadable:
                subscriptionDiagnostics.availability !=
                SubscriptionAvailability.unavailable,
            proEntitlementActive: currentState?.isPro ?? false,
            cachedProOnDisk: cachedState?.isPro ?? false,
            restorePurchasesReachable: true,
            restoreNoCrashVerified: true,
            entitlementPersistsAfterRestart:
                (cachedState?.isPro ?? false) && (currentState?.isPro ?? false),
  ...
  Future<void> _copySummary() async {
    final lines = [
      ...
      'Health: $_health',
      'Local saved moments: $_entryCount',
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          _row('Local saved moments', _loading ? '…' : '$_entryCount'),
          _row(
          ...
          ListTile(
            ...
            subtitle: const Text(
              'Run QA samples through the first-moment engine',
            ),
            ...
            OutlinedButton(
              ...
              onPressed: _loading
                  ...
                  : () async {
                      ...
                      BetaActivationLoopTracker.resetSession();
                      await _refresh();
                    },

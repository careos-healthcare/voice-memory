...
class _TrialControlScreenState extends State<TrialControlScreen> {
  ...
  SubscriptionDiagnostics? _subscriptionDiagnostics;
  ...
  Future<void> _refresh() async {
    ...
    final diagnostics = await AppServices.instance.subscriptionRepository
        .loadDiagnostics();
    if (!mounted) return;
    setState(() {
      _summary = summary;
      ...
      _subscriptionDiagnostics = diagnostics;
      _loading = false;
    });
  }
  ...
  List<Widget> _metricRows(TrialSummaryModel s) {
    ...
    return [
      _row('First moment saved', '${s.firstReflectionSaved}'),
      ...
      _row('Second moment saved', '${s.secondReflectionSaved}'),
      ...
      _row('Third moment saved', '${s.thirdReflectionSaved}'),
      ...
      _row('Moments saved', '${s.recordingSavedCount}'),
  ...
  List<Widget> _positioningComprehensionRows(TrialSummaryModel s) {
    ...
    return [
      ...
      _row('Archive', '${s.positioningJournalCount}'),
  ...
  List<Widget> _subscriptionDiagnosticRows() {
    final d = _subscriptionDiagnostics;
    return [
      _row('availability', d?.availability.name ?? 'loading'),
      _row('offersLoaded', '${d?.offersLoaded ?? false}'),
      _row('offerCount', '${d?.offerCount ?? 0}'),
      _row('lastError', d?.lastError ?? '—'),
    ];
  }

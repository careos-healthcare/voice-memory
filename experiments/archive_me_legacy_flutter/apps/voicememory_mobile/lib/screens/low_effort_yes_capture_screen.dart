...
import '../widgets/life_os/post_save_life_os_insights_card.dart';
...
class LowEffortYesCaptureScreen extends StatefulWidget {
  ...
  final PostSaveLifeOsInsightsEntryLoader? insightsLoader;
...
class _LowEffortYesCaptureScreenState extends State<LowEffortYesCaptureScreen> {
  ...
  void initState() {
    super.initState();
    if (widget.instantMode) {
      _capacityWedgeActive = true;
  ...
  Future<void> _save() async {
    ...
    setState(() {
      _saving = false;
      _savedEntryId = saveResult.entryId;
  ...
  Future<PostSaveLifeOsInsights> _loadPostSaveInsights(String entryId) async {
    final injected = widget.insightsLoader;
    if (injected != null) return injected(entryId);
    final entries = await AppServices.instance.journalStore.loadAll();
    return PostSaveLifeOsInsightsService().analyze(
      entries: entries,
      finalizedEntryId: entryId,
    );
  }
      ...
      return PushedScreenShell(
        ...
        body: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              QuickCaptureFrictionCard(
                result: frictionResult,
                onSaved: () => unawaited(_dismissAfterFriction()),
              ),
              if (_savedEntryId case final entryId?) ...[
                const SizedBox(height: AppSpacing.md),
                PostSaveLifeOsInsightsCard(
                  entryId: entryId,
                  loader: () => _loadPostSaveInsights(entryId),
                ),
              ],
            ],
          ),

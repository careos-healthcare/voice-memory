...
import '../features/archive_memory/memory_quality_coordinator.dart';
import '../features/archive_memory/memory_quality_model.dart';
import '../features/pattern_profile/pattern_profile_coordinator.dart';
import '../features/pattern_profile/pattern_profile_model.dart';
...
import '../widgets/feedback/archive_feedback_chips.dart';
import '../widgets/patterns/memory_quality_chip.dart';
...
class _PatternProfileScreenState extends State<PatternProfileScreen> {
  ...
  Widget _content(PatternProfile profile) {
    ...
    final clarityLabel = profile.clarityLabel;
    final memorySummary = profile.archiveMemorySummary;
    final nextCheck = profile.nextCheck?.trim() ?? '';
    return ListView(
      padding: const EdgeInsets.all(AppSpacing.md),
      children: [
        Text(
        ...
        if (quality != null && quality.shouldShow) ...[
          const SizedBox(height: AppSpacing.xs),
          MemoryQualityChip(quality: quality),
        ] else if (clarityLabel != null && clarityLabel.trim().isNotEmpty) ...[
          const SizedBox(height: AppSpacing.xs),
          Text(
            clarityLabel,
        ...
        if (memorySummary != null) ...[
          _section('What ArchiveMe remembers', memorySummary.primaryMemoryLine),
        ...
        if (nextCheck.isNotEmpty) ...[
          _section('Next check', nextCheck, emphasize: true),
          const SizedBox(height: AppSpacing.md),
          SizedBox(
            width: double.infinity,
            height: 48,
            child: FilledButton(
              onPressed: _checkSet ? null : () => _useCheck(nextCheck),
              child: Text(
                _checkSet
  ...
  Widget _timelinePreview(PatternProfile profile) {
    final timeline = profile.archiveEvolutionTimeline;
    if (timeline == null) return const SizedBox.shrink();
    final events = timeline.events.take(3);
    final lines = events

...
class DailyArchiveMemoryCard extends StatefulWidget {
  ...
  final VoidCallback? onDontRemindAgain;
...
class _DailyArchiveMemoryCardState extends State<DailyArchiveMemoryCard> {
  ...
  void _onDontRemindAgain() {
    DailyArchiveMemoryAnalytics.ctaTapped(
      source: widget.source,
      entryCount: widget.entryCount,
      actionType: 'dont_remind_again',
    );
    widget.onDontRemindAgain?.call();
    if (!mounted) return;
    setState(() => _dismissedToday = true);
  }
        ...
        children: [
          if (focused && watchPhrase != null) ...[
            Text(
              ...
              key: const Key('daily_archive_memory_watch_headline'),
            ...
            Text(
              DailyArchiveMemoryCopy.watchSubtext(watchPhrase),
          ...
          if (focused && widget.onDontRemindAgain != null)
            Align(
              alignment: Alignment.center,
              child: TextButton(
                key: const Key('daily_archive_memory_dont_remind_again_cta'),
                onPressed: _onDontRemindAgain,
                style: TextButton.styleFrom(
                  foregroundColor: AppColors.textSecondary,
                  minimumSize: const Size(48, 44),
                ),
                child: Text(DailyArchiveMemoryCopy.dontRemindAgainCta),
              ),
            ),
          if (!focused && widget.onRecord != null) ...[

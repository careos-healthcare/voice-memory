...
class _PostSaveInsightChoiceCardState extends State<PostSaveInsightChoiceCard> {
  ...
  final Set<String> _recordedConclusionIds = {};
  ...
  void initState() {
    ...
    for (final signal in _bundle.signals) {
      unawaited(_recordConclusionOnce(signal));
    }
  ...
  Future<void> _recordConclusionOnce(PostSaveInsightSignal signal) async {
    final conclusion = signal.explainableConclusion;
    final entry = widget.entry;
    if (conclusion == null ||
        entry == null ||
        !_recordedConclusionIds.add(conclusion.id) ||
    ...
    final gated = ExplainableConclusionRenderGate.visible(
      ...
      canonicalTranscripts: {
        for (final item in [...widget.priorEntries, entry])
          item.id: item.transcript,
      },
    );
    if (gated == null) {
      _recordedConclusionIds.remove(conclusion.id);
      return;
    }
    await AppServices.instance.explainabilityHistoryStore.appendIfAbsent(gated);
  }
  ...
  void _showAlternative(PostSaveInsightSignal from) {
    ...
    if (next == null) return;
    unawaited(_recordConclusionOnce(next));
  ...
  Widget _buildAuditableV1(BuildContext context) {
    final canonicalTranscripts = {
      for (final item in [...widget.priorEntries, widget.entry!])
        item.id: item.transcript,
    };
    final conclusion = _bundle.signals.firstOrNull?.explainableConclusion;
    ...
    final nextQuestion =
        gated?.value.nextRecordingPrompt?.trim().isNotEmpty == true
        ? gated!.value.nextRecordingPrompt!.trim()
        : (_bundle.clearerMomentPrompt?.trim().isNotEmpty == true
              ? _bundle.clearerMomentPrompt!.trim()
              : 'What happened next, and what felt different or the same?');
    return ArchiveResponsiveLayout.constrainContent(
      context: context,
      child: Column(
        key: const Key('post_save_auditable_conclusion_section'),
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          if (gated != null)
            ExplainableConclusionCard(
              conclusion: gated,
              analyticsOrigin: 'post_save',
          ...
          Text(
            nextQuestion,
            key: const Key('post_save_auditable_next_question'),
            style: ArchiveMobileTypography.explanationBody(context),
          ),
          const SizedBox(height: AppSpacing.sm),
          FilledButton.icon(
            key: const Key('post_save_auditable_record_next'),
            onPressed: () {
              widget.onRecordNextEvidence?.call(nextQuestion);
              widget.onRecordNext();
            },
  ...
  Widget build(BuildContext context) {
    // V1 renders conclusions only through the canonical aggregate, validator,
    // shared receipt UI, and shared feedback coordinator.
    if (widget.entry != null) return _buildAuditableV1(context);
    return const EvidenceReceiptUnavailableState();
    // ignore: dead_code
    final impossibleInsightCard = _impossibleInsightCard();
    if (impossibleInsightCard != null) {
      return ArchiveResponsiveLayout.constrainContent(
        context: context,
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            impossibleInsightCard,
            const SizedBox(height: AppSpacing.sm),
    ...
    final cloudConclusionCard = _cloudConclusionCard();
    if (cloudConclusionCard != null) {
      return ArchiveResponsiveLayout.constrainContent(
        context: context,
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
  ...
  Widget? _impossibleInsightCard() {
    final insight = _bundle.impossibleInsight;
    final entry = widget.entry;
    if (insight == null || entry == null) return null;
    final transcripts = {
      for (final item in [...widget.priorEntries, entry])
        item.id: item.transcript,
    };
    ...
    return Column(
      key: const Key('impossible_insight_card'),
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        ExplainableConclusionCard(
          conclusion: gated,
        ...
        const SizedBox(height: AppSpacing.xs),
        Text(
          insight.nextEvidenceQuestion,
          key: const Key('impossible_insight_next_question'),
          style: ArchiveMobileTypography.explanationBody(context),
        ),
      ],
    );
  ...
  Widget? _cloudConclusionCard() {
    ...
    return ExplainableConclusionCard(
      ...
      onShowHistory: () async {
        if (!AppServices.isInitialized) return;
        final history = await AppServices.instance.explainabilityHistoryStore
            .byConclusionId(gated.value.id);
        if (!mounted) return;
        await ExplainableHistorySheet.show(
          context,
...
class _AbChoiceView extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    if (pair.optionA.explainableConclusion == null ||
        pair.optionB.explainableConclusion == null) {
...
class _AbOptionCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    if (signal.explainableConclusion == null) {
      return const SizedBox.shrink();
    }
    final gap = ArchiveResponsiveLayout.gap(context);
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        if (signal.recordNextQuestion.trim().isNotEmpty) ...[
          const SizedBox(height: AppSpacing.sm),
          Text(
            'Record next',
            style: ArchiveMobileTypography.cardLabel(context),

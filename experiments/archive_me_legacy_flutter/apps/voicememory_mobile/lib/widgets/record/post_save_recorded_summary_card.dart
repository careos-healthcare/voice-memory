...
class PostSaveRecordedSummaryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final firstInsight = _isFirstSavedEntry
        ? buildFirstRecordingInsight(_entries)
        : null;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(PostSaveRecordedSummaryCopy.title, style: labelStyle),
          const SizedBox(height: AppSpacing.xs),
          Text(
            summary,
          ...
          if (_isLowSignal && _shows(PostSavePrimaryArchiveKind.lowSignal)) ...[
            ...
            const SizedBox(height: AppSpacing.md),
            Text(
              'What ArchiveMe noticed',
              key: const Key('post_save_first_observation_title'),
              style: ArchiveMobileTypography.responsiveSectionTitle(context),
            ),
            const SizedBox(height: AppSpacing.xs),
            if (!_isLowSignal &&
                firstInsight?.firstObservation != null &&
                firstInsight?.strongestQuote != null) ...[
              Text(
                firstInsight!.firstObservation!,
                key: const Key('post_save_first_observation'),
                style: bodyStyle,
              ),
              const SizedBox(height: AppSpacing.sm),
              Text('Supporting words', style: labelStyle),
              const SizedBox(height: 4),
              Text(
                '“${firstInsight.strongestQuote!}”',
                key: const Key('post_save_first_observation_evidence'),
                style: footnoteStyle,
              ),
            ] else
              Text(
                'There is not enough detail for a reliable observation yet. '
                'Your transcript is saved exactly as captured.',
                key: const Key('post_save_first_observation_low_evidence'),
                style: footnoteStyle,
              ),
            const SizedBox(height: AppSpacing.sm),
          ...
          if (_isFirstSavedEntry && onAddContext != null) ...[
            const SizedBox(height: AppSpacing.sm),
            TextButton.icon(
              key: const Key('post_save_add_context'),
              onPressed: onAddContext,
              icon: const Icon(Icons.person_add_alt_1_outlined),
              label: const Text('Add context'),
            ),
          ],
          if (_isFirstSavedEntry && onBackToRecord != null) ...[
            const SizedBox(height: AppSpacing.xs),
            FilledButton.icon(
              key: const Key('post_save_record_another'),
              onPressed: onBackToRecord,
              icon: const Icon(Icons.mic_none),
              label: const Text('Record another moment'),
            ),
          ],
          if (_isFirstSavedEntry && onViewArchive != null) ...[
            const SizedBox(height: AppSpacing.xs),
            OutlinedButton.icon(
              key: const Key('post_save_view_archive'),
              onPressed: onViewArchive,
              icon: const Icon(Icons.book_outlined),
              label: const Text('View in Archive'),
            ),
          ],
          const SizedBox(height: AppSpacing.sm),
...
class _DegradedTranscriptionCardState
  ...
  String? get _audioPath {
    final trimmed = widget.entry.localAudioReference ?? '';

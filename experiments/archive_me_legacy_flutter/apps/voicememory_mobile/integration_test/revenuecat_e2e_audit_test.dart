...
void main() {
  ...
  testWidgets('RevenueCat E2E runtime audit', (tester) async {
    ...
    final subscriptions = AppServices.instance.subscriptionRepository;
    String entitlementLabel(SubscriptionState state) =>
        '${state.tier.name}; pro=${state.isPro}; ids=${state.entitlementIds.join(",")}; source=${state.origin.name}';

    log('platform=${Platform.operatingSystem}');
    log('sdk_configured=${rc.isConfigured}');

    SubscriptionState ent = SubscriptionState.free();
    try {
      ent = await subscriptions.refresh(force: true);
    } catch (e) {
    ...
    for (final spec in [
      ...
      if (spec.$1 == 12) {
        ...
        log(
          'offline_cache=${cached != null ? "${cached.tier.name}; pro=${cached.isPro}; ids=${cached.entitlementIds.join(",")}; source=${cached.source}" : "empty"}',
        );
        notes =

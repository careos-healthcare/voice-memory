...
void main() {
  ...
  group('WhatChangedV2Engine gates', () {
    test('one entry waits while two related entries can compare', () {
      expect(
      ...
      final related = _threeSaidYesEntries().take(2).toList();
      expect(WhatChangedV2Engine.buildPrompt(entries: related), isNotNull);
      expect(
        WhatChangedV2Engine.buildPrompt(
          entries: [
            _voiceEntry(
              id: 'work',
              transcript: 'I checked the work deadline and sent the report.',
              createdAt: DateTime(2026, 6, 10),
            ),
            _voiceEntry(
              id: 'family',
              transcript:
                  'I enjoyed dinner with my sister and discussed music.',
  ...
  group('dedup and isolation', () {
    test('record screen does not import legacy return check answer card', () {
      ...
      expect(
        dependencies.contains('post_save_return_check_answer_card'),
        isFalse,
      );
      expect(controller.contains('PostSaveReturnCheckAnswerCard'), isFalse);
      expect(dependencies.contains('what_changed_v2_card'), isTrue);
      expect(controller.contains('WhatChangedV2Card'), isTrue);
    });
    ...
    test('comparison requires grounded prior and latest evidence', () {
      ...
      final comparison = WhatChangedV2Engine.buildComparison(entries: entries);

...
void main() {
  ...
  test('pack includes theory and reflection index', () {
    ...
    expect(pack['engine'], 'archive_intelligence');
    expect(pack['eligibleCount'], 50);
    expect(pack['theory'], isNotNull);
    expect(pack['primaryTheory'], isNotNull);
    final slices = pack['slices'] as Map<String, dynamic>;
    expect(slices.keys, containsAll(['theory', 'change', 'patterns']));
    expect(pack['reflectionIndex'], hasLength(50));
    final firstRow =
        (pack['reflectionIndex'] as List).first as Map<String, dynamic>;
    final source = entries.firstWhere((entry) => entry.id == firstRow['id']);
    expect(firstRow['canonicalTranscript'], source.transcript);

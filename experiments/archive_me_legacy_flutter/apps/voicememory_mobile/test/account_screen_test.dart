...
import 'package:voicememory_mobile/widgets/account/pro_utility_expansion_section.dart';
...
void main() {
  ...
  testWidgets('shows model card between pro utility and weekly growth', (
    tester,
  ) async {
    ...
    final modelCard = find.byKey(const Key('llama_model_download_card'));
    final weeklyCard = find.byKey(const Key('weekly_growth_preview_card'));
    expect(modelCard, findsOneWidget);
    expect(
      find.byKey(const Key('llama_model_download_source_account')),
      findsOneWidget,
    );
    expect(
      tester.getTopLeft(find.byType(ProUtilityExpansionSection)).dy,
      lessThan(tester.getTopLeft(modelCard).dy),
    );
    expect(
      tester.getTopLeft(modelCard).dy,
      lessThan(tester.getTopLeft(weeklyCard).dy),
    );
  });

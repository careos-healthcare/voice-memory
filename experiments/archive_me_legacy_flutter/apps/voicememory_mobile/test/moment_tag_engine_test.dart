...
void main() {
  test('matches context keywords conservatively', () {
    expect(
      buildMomentTags('A long day at work and the meeting ran over.'),
      contains('work'),
    );
    expect(
      buildMomentTags('I called my mum about the kids.'),
      contains('family'),
    );
  ...
  test('adds result-driven tags from the hint, not the text', () {
    expect(
      buildMomentTags('A quiet day.', resultHint: 'lighter'),
      contains('lighter'),
    );
    expect(
      buildMomentTags('A quiet day.', resultHint: 'heavier'),
      contains('heavier'),
    );
    expect(
      buildMomentTags('A quiet day.', resultHint: 'changed'),
      contains('changed'),
    );
    expect(
      buildMomentTags('A quiet day.', resultHint: 'none_fit'),
      contains('changed'),
    );
  });
  ...
  test('result tag is not duplicated when capped', () {
    final tags = buildMomentTags(
      'work family partner friend money',
      resultHint: 'heavier',
      max: 5,
    );

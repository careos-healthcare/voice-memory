...
void main() {
  group('ArchiveAnalystEngine', () {
    ...
    test('builds an evidence-bound audio digest', () {
      const report = ArchiveAnalystReport(
        level: ArchiveAnalystLevel.level1,
        eligibleReflectionCount: 52,
        evidenceSummary: ArchiveAnalystEvidenceSummary(
          eligibleReflectionCount: 52,
          dateSpanLabel: 'January to March',
        ...
        currentBeliefs: [
          ArchiveAnalystBeliefRow(
            id: 'primary',
            statement: 'I protect my energy before saying yes',
      ...
      final digest = const ArchiveAnalystEngine().buildAudioDigest(report);

      expect(digest, startsWith('Archive Audio Digest.'));
      expect(digest, contains('52 saved moments'));
      expect(digest, contains('8 supporting moments'));
      expect(digest, contains('not a diagnosis'));
    });
  });
}

...
import 'package:voicememory_mobile/features/curiosity_loop/curiosity_hook_gates.dart';
...
import 'package:voicememory_mobile/features/curiosity_loop/services/curiosity_hook_metadata_extractor.dart';
...
void main() {
  ...
  group('CuriosityHookCoordinator', () {
    ...
    test(
      'persists hook without scheduling a legacy curiosity reminder',
      () async {
        final entries = _confirmedRepeatEntries(3);
        final scheduler = _FakeCuriosityNotificationScheduler(
    ...
    test(
      'does not request notification permission from curiosity hook path',
      () async {
        final entries = _confirmedRepeatEntries(3);
        final scheduler = _FakeCuriosityNotificationScheduler(
    ...
    test(
      'persists synthesized prompt without a separate notification',
      () async {
        final entries = _confirmedRepeatEntries(3);
  ...
  group('CuriosityHookGates', () {
    test('shows post-save card for fresh unconsumed hook', () {
      final hook = CuriosityHook(
        id: 'h1',
        entryId: 'e1',
        createdAt: DateTime.utc(2026, 6, 12),

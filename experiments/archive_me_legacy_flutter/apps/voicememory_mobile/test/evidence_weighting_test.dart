...
List<JournalEntry> _softeningEntries() => [
  ...
  _entry(
    '4',
    'Said yes again when I had no capacity but it felt easier to stop this time.',
...
void main() {
  ...
  group('Evidence weighting placement', () {
    test('focused Archive home delegates to one canonical composition', () {
      final source = File(
        'lib/screens/archive_belief_screen.dart',
      ).readAsStringSync();
      expect(source, contains('ArchiveIntelligencePresentation.build'));
      expect(source, contains('ArchiveIntelligenceHome'));
      expect(source, isNot(contains('EvidenceWeightingCard(')));
    });
    ...
    test('focused Archive home does not duplicate legacy evidence cards', () {
      final source = File(
        'lib/screens/archive_belief_screen.dart',
      ).readAsStringSync();
      expect(source, isNot(contains('CurrentRelevanceCard(')));
      expect(source, isNot(contains('EvidenceWeightingCard(')));
    });
  });

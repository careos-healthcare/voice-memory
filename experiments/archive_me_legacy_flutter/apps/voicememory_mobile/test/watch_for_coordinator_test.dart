...
void main() {
  ...
  test(
    'returns Pro bridge result before replacing a free active target',
    () async {
      final stamp = DateTime.now().microsecondsSinceEpoch.toString();
      await _reset(stamp);
      final first = WatchForCoordinator.buildSuggestedWatchForAfterSave(
        entries: [
          _entry('I keep taking responsibility before asking for help'),
        ],
        now: DateTime(2026, 5, 25),
      );
      final second = first.copyWith(id: 'second-watch');

      await WatchForCoordinator.acceptSuggestedWatchFor(
        first,
        entitlementReader: FakeArchiveEntitlementReader(pro: false),
      );
      final result = await WatchForCoordinator.acceptSuggestedWatchFor(
        second,
        entitlementReader: FakeArchiveEntitlementReader(pro: false),
      );

      expect(result.requiresPro, isTrue);
      expect(
        (await WatchForStore(
          AppServices.instance.prefs,
        ).readActive()).map((item) => item.id),
        [first.id],
      );
    },
  );

  test('allows Pro to accept a second active target', () async {
    final stamp = DateTime.now().microsecondsSinceEpoch.toString();
    await _reset(stamp);
    final first = WatchForCoordinator.buildSuggestedWatchForAfterSave(
      entries: [_entry('I keep taking responsibility before asking for help')],
    ...
    final second = first.copyWith(id: 'second-watch');
    final pro = FakeArchiveEntitlementReader(pro: true);

    await WatchForCoordinator.acceptSuggestedWatchFor(
      first,
      entitlementReader: pro,
    );
    final result = await WatchForCoordinator.acceptSuggestedWatchFor(
      second,
      entitlementReader: pro,
    ...
    expect(result.requiresPro, isFalse);
    expect(
      (await WatchForStore(AppServices.instance.prefs).readActive()).length,
      2,
    );
  });

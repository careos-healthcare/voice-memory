...
List<JournalEntry> _unrelatedTwoEntries() => [
  _entry(
    id: 'u1',
    transcript: 'Monday standup ran long and I skipped lunch entirely.',
    createdAt: DateTime(2026, 6, 10, 12),
  ),
  _entry(
    id: 'u2',
    transcript: 'The kitchen tap has been dripping since last week.',
...
void main() {
  ...
  group('Archive/Patterns ladder', () {
    ...
    test(
      ...
      () {
        final entries = _unrelatedTwoEntries();
        ...
        expect(fourState?.state, ArchiveTabFourState.twoUnrelated);
        expect(home.title, isEmpty);
        expect(home.body, ArchiveTabFourStateCopy.twoUnrelatedBody);
        expect(home.body.toLowerCase(), isNot(contains('pattern found')));
        expect(home.footnoteLine, isNull);
  ...
  group('First-run copy quality', () {
    ...
    test('first-run copy uses ArchiveMe not VoiceMemory', () {
      ...
      expect(
        VisibleArchiveProofCopy.firstSaveSecondary.toLowerCase(),
        isNot(contains('voicememory')),
      );
      expect(

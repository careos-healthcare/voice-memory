...
void main() {
  ...
  group('PaywallAccess delayed paywall gate', () {
    ...
    testWidgets('free export stays available before paywall can open', (
      tester,
    ) async {
      ...
      final allowed = await tester.runAsync(
        () => PaywallAccess.ensureProAccess(
          testContext,
          capability: CapabilityId.exportOriginalContent,
          entitlementReader: FakeArchiveEntitlementReader(pro: false),
          sourceRoute: '/archive-export',
        ),
      );
      expect(allowed, isTrue);
    });

    testWidgets('Pro export remains available without opening a paywall', (
      tester,
    ) async {
      late BuildContext testContext;
      await tester.pumpWidget(

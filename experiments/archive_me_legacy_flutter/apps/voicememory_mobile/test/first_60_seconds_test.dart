...
void main() {
  ...
  group('Copy guardrails', () {
    test('intro copy is exact', () {
      expect(First60Copy.introTitle, 'Your voice becomes your life story');
    ...
    test('Pro bridge copy is exact', () {
      expect(First60Copy.proTitle, ProSinglePromiseCopy.headline);
      expect(
        First60Copy.proBody,
  ...
  group('First archive view', () {
      ...
      await pumpUntil(
        ...
        () => entryCount == 1
            ? find
                  ...
                  .evaluate()
                  .isNotEmpty
            : find.byTooltip('Delete local entry').evaluate().length ==
                  entryCount,
        reason: 'JournalScreen to render $entryCount entries',
      );
    }
    ...
    testWidgets('helper appears for exactly one entry with both actions', (
      ...
      await pumpJournal(tester, entryCount: 1);
    ...
    testWidgets('helper hides with two entries', (tester) async {
      ...
      await pumpJournal(tester, entryCount: 2);
    ...
    testWidgets('pin action pins the single entry via the existing flow', (
      ...
      await tester.runAsync(() async {
        ...
        fail('Pin action did not persist after 50 event-loop turns');
      });
    ...
    testWidgets('Pro bridge shows after repeat value and Not now persists', (
      ...
      await tester.runAsync(() async {
        await AppServices.instance.journalStore.save(
          _entry(
            id: 'a',
            transcript:
                'I took responsibility before asking for help at work today.',
          ),
        );
        await AppServices.instance.journalStore.save(
          _entry(
            ...
            transcript:
                'I took responsibility before asking for help at work again.',
          ),
        );
      });
      late List<JournalEntry> savedEntries;
      await tester.runAsync(() async {
        savedEntries = await AppServices.instance.journalStore.loadAll();
      });
      expect(
        PaywallTimingGates.hasArchiveProofFromEntries(entries: savedEntries),
        isTrue,
      );
      late RecordReturnProState initialLoopState;
      late bool isPro;
      await tester.runAsync(() async {
        initialLoopState = await RecordReturnProStore.instance().load();
        isPro =
            (await AppServices.instance.subscriptionRepository.refresh()).isPro;
      });
      expect(initialLoopState.proBridgeResolved, isFalse);
        ...
        MaterialApp(
          home: Scaffold(
            body: ProValueClarityCard(
              ...
              onNotNow: () {
                unawaited(
                  RecordReturnProStore.instance().markProBridgeResolved(),
                );
              },
            ),
          ),
      ...
      await tester.runAsync(() async {
        notNow.onPressed!();
        for (var attempt = 0; attempt < 50; attempt++) {
          if ((await RecordReturnProStore.instance().load())
              .proBridgeResolved) {
            return;
          }
          await Future<void>(() {});
        }
        fail('Pro bridge dismissal did not persist');
      });
      await tester.pump();
      expect(

...
import 'package:voicememory_mobile/features/identity_engine/identity_engine.dart';
import 'package:voicememory_mobile/features/identity_engine/identity_models.dart';
...
void main() {
    ...
    final profile = IdentityProfile(
      currentTraits: [
        IdentityTrait(
          ...
          confidence: 70,
          evidenceCount: 3,
          supportingRecordingIds: const ['a', 'b'],
          supportingQuotes: const ['I avoid hard talks'],
  ...
  test('legacy profile parsing skips malformed traits and values', () {
    final parsed = IdentityProfile.fromJson({
      'currentTraits': [
        null,
        'not-a-map',
        {
          'id': 'theme:approval',
          'title': 'Approval is becoming less central',
          'confidence': '72',
          'evidenceCount': '3',
          'supportingRecordingIds': null,
          'supportingQuotes': 'legacy-string',
          'trend': 'down',
        },
      ],
      'emergingTraits': 'invalid-list',
      'evidenceReflectionCount': '5',
    });

    expect(parsed.currentTraits, hasLength(1));
    expect(parsed.currentTraits.single.confidence, 72);
    expect(parsed.currentTraits.single.evidenceCount, 3);
    expect(parsed.currentTraits.single.supportingQuotes, isEmpty);
    expect(parsed.emergingTraits, isEmpty);
    expect(parsed.evidenceReflectionCount, 5);
  });

  test('legacy discover baseline ignores invalid values', () {
    final parsed = ThemeTrackerService.parseDiscoverBaseline({
      'approval': '2',
      'confidence': 3.8,
      'career': null,
      'health': 'not-a-number',
    ...
    expect(parsed, {'approval': 2, 'confidence': 3});
  });
}

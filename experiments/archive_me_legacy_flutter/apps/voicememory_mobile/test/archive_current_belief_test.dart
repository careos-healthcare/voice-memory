...
void main() {
  ...
  group('Patterns post-proof stack order', () {
    ...
    test(
      'Archive Intelligence renders its four sections in canonical order',
      () {
        final source = File(
          'lib/features/archive_home/archive_intelligence_home.dart',
        ).readAsStringSync();
        final whatChanged = source.indexOf('WhatChangedSection(');
        final reasoning = source.indexOf('ArchiveReasoningSection(');
        final moments = source.indexOf('SupportingMomentsSection(');
        final nextAction = source.indexOf('ArchiveNextActionSection(');

        expect(whatChanged, greaterThan(0));
        expect(reasoning, greaterThan(whatChanged));
        expect(moments, greaterThan(reasoning));
        expect(nextAction, greaterThan(moments));
      },
    );

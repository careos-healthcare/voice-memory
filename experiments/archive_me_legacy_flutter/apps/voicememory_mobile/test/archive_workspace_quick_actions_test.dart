...
import 'package:voicememory_mobile/widgets/archive/archive_workspace_quick_actions_card.dart';
...
void main() {
  ...
  group('ArchiveWorkspaceQuickActionsEngine', () {
    ...
    test('2 entries with untagged usable entries shows Tag untagged entries', () {
      ...
      expect(
        _kinds(actions),
        contains(ArchiveWorkspaceQuickActionKind.tagUntagged),
      );
      expect(
    ...
    test(
      ...
      () {
        ...
        final archiveHome = ArchiveHomeSummaryEngine.build(entries: entries);
        final actions = _quickActions(entries);
        if (archiveHome.primaryAction == ArchiveHomeAction.addMoment &&
            archiveHome.primaryCta ==
    ...
    test('V1 hides Review corrections quick action', () {
    ...
    test('V1 hides View weekly review quick action', () {
      final actions = _quickActions(_distinctWorkEntries(5));
      expect(V1FeatureFlags.enableCustomReports, isFalse);
      expect(
        _kinds(actions),
        isNot(contains(ArchiveWorkspaceQuickActionKind.viewWeeklyReview)),
      );
    });

...
void main() {
  ...
  group('Remember this button', () {
    testWidgets('stays hidden on entry detail while action items are off', (
      tester,
    ) async {
      late Directory dir;
      await tester.runAsync(() async {
  ...
  group('Privacy', () {
    test('entries and preferences use mocked secure storage', () async {
      await store.create(
        sourceEntryId: 'e1',
        title: _privateTitle,
        note: _privateNote,
      );
      await journal.save(_entry(id: 'e1'));

      expect(File('${tmp.path}/prefs.json').existsSync(), isFalse);
      expect(File('${tmp.path}/entries.json').existsSync(), isFalse);
      expect(

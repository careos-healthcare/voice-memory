...
void main() {
  ...
  group('Session 1 confirmation', () {
    testWidgets('first save evidence card shows focused copy', (tester) async {
      ...
      expect(
        find.textContaining('If it returns, changes, fades, or disappears'),
        findsOneWidget,
      );
      expect(
      ...
      expect(
        find.text(RecordReturnProCopy.evidenceRecordAnother),
        findsOneWidget,
      );
  ...
  group('Session 3 archive usefulness', () {
    ...
    testWidgets('patterns card shows thread copy', (tester) async {
      ...
      expect(find.text(FirstThreeSessionCopy.session3Title), findsOneWidget);
      expect(
        find.text(FirstThreeSessionCopy.session3KeepsReturning),
        findsOneWidget,
      );
      expect(
        find.text(FirstThreeSessionCopy.session3ChangedSince),
        findsOneWidget,
      );
    });
  });
  ...
  group('Pro boundary', () {
    testWidgets('soft Pro card uses continuity copy after value', (
      ...
      expect(find.text(RecordReturnProCopy.proTitle), findsOneWidget);
      expect(find.text(RecordReturnProCopy.proBody), findsOneWidget);
      expect(find.text(RecordReturnProCopy.proContinuityLine), findsOneWidget);
  ...
  group('FirstThreeJourneyEngine alignment', () {
    ...
    test(
      'three entries without grounded repeat keeps useful-archive title',
      () {
        final model = const FirstThreeJourneyEngine().build(
          reflectionCount: 3,
          entries: [
            _entry(
              'a',
              'Monday standup ran long and I skipped lunch entirely.',
            ),
            _entry('b', 'The kitchen tap has been dripping since last week.'),
            _entry('c', 'Finished reading a novel before bed tonight.'),
          ],
        );

...
void main() {
  ...
  group('ProveEnoughEvidenceTrailScreen', () {
    testWidgets('free user sees the complete existing evidence trail', (
      tester,
    ) async {
      final trail = _sampleTrail();
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: ProveEnoughEvidenceTrailScreen(
            initialTrail: trail,
            initialEntitlements: SubscriptionState.free(),
          ),
        ),
      );
    ...
    testWidgets('existing evidence never exposes an upgrade CTA', (
      tester,
    ) async {
      final trail = _sampleTrail();
      await tester.pumpWidget(
      ...
      expect(
        find.byKey(const Key('prove_enough_trail_see_pro_cta')),
        findsNothing,
      );

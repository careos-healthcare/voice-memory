...
void main() {
  ...
  group('Record screen first-session ladder', () {
    ...
    Future<void> seedEntries(WidgetTester tester, int count) async {
      ...
      await tester.runAsync(() async {
        await AppServices.instance.journalStore.replaceAll(
          List.generate(count, (i) => usableEntry(id: 'seed_$i')),
        );
      });
    }
    ...
    testWidgets('first save shows transcript-first evidence hierarchy', (
      tester,
    ) async {
      ...
      expect(
        find.byKey(const Key('post_save_first_observation_title')),
        findsOneWidget,
      );
      expect(find.byKey(const Key('post_save_add_context')), findsOneWidget);
      expect(find.byKey(const Key('post_save_record_another')), findsOneWidget);
  ...
  group('Record screen return-path gates', () {
    ...
    testWidgets(
      'zero and one entry ready screens hide two-day activation card',
      (tester) async {
        await pumpRecordScreen(tester);
        expect(find.byKey(const Key('two_day_activation_card')), findsNothing);

...
void main() {
  ...
  group('OfflineVaultRecoveryStore', () {
    ...
    test(
      'registerVault keeps encrypted file until server ack completes',
      () async {
        final source = File(
          '${vaultDirectory.path}/audio_vault_session_1.vault.enc',
        );
    ...
    test('markCompleted deletes vault file after recovery ack', () async {
      final source = File(
        '${vaultDirectory.path}/audio_vault_session_2.vault.enc',
      );
    ...
    test(
      'discoverOrphans registers unknown vault.enc files as pending jobs',
      () async {
        final orphan = File(
          '${vaultDirectory.path}/audio_vault_session_3.vault.enc',
        );
    ...
    test(
      'registerVault marks offline_* recoverable only when recovery secret present',
      () async {
        final source = File(
    ...
    test('discard removes vault artifacts without server ack', () async {
      final source = File(
        '${vaultDirectory.path}/audio_vault_session_4.vault.enc',
      );

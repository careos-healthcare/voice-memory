...
OfflineVaultRecoveryStore _testRecoveryStore(Directory vaultDirectory) {
  return OfflineVaultRecoveryStore(
    manifestFile: File('${vaultDirectory.path}/offline_vault_manifests.json'),
...
void main() {
  group('LiveVoiceCaptureService + IsolateAudioPipeline Bridge Tests', () {
    ...
    late Directory bridgeVaultDirectory;
    ...
    setUp(() async {
      ...
      bridgeVaultDirectory = await Directory.systemTemp.createTemp(
        'live_voice_bridge_vault_',
      );
      pcmSent = <List<int>>[];
      journalFile = File(
      ...
      captureService = _buildBridgeCaptureService(
        ...
        pcmSent: pcmSent,
        testVaultDirectory: bridgeVaultDirectory,
      );
    });
    ...
    tearDown(() async {
      ...
      if (bridgeVaultDirectory.existsSync()) {
        await bridgeVaultDirectory.delete(recursive: true);
      }
      ApiUsageGuard.resetForTest();
    });
    ...
    test(
      ...
      () async {
        ...
        final bridgeService = _buildBridgeCaptureService(
          ...
          recoveryStore: _testRecoveryStore(vaultDirectory),
        );
  ...
  group('LiveVoiceCaptureService', () {
    ...
    setUp(() async {
      ...
      service = LiveVoiceCaptureService(
        ...
        reconnectRetryPolicy: const RetryPolicy(
          maxAttempts: 1,
    ...
    test(
      ...
      () async {
        ...
        service = _buildBridgeCaptureService(
          ...
          testVaultDirectory: vaultDirectory,
        );
...
LiveVoiceCaptureService _buildBridgeCaptureService({
  ...
  Directory? testVaultDirectory,
}) {
  final vaultDirectory = testVaultDirectory;
  return LiveVoiceCaptureService(
    ...
    recoveryStore:
        ...
        _testRecoveryStore(
          vaultDirectory ??
              (throw StateError('Provide recoveryStore or testVaultDirectory')),
        ),
    reconnectRetryPolicy: const RetryPolicy(
      maxAttempts: 1,
      initialDelay: Duration.zero,
...
class _RecordingPipeline extends CapturePipelineService {
  ...
  LocalCaptureContext? lastLocalContext;
  ...
  Future<CapturePipelineResult> saveLiveVoiceTranscript({
    ...
    lastLocalContext = localCaptureContext;

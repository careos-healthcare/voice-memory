...
import 'fixtures/comparison_output_parser_fixtures.dart';
...
''';
  ...
  group('ComparisonOutputParser fixture recovery', () {
    for (final fixture in comparisonOutputParserFixtures) {
      test(fixture.name, () {
        final result = parser.parse(fixture.input);

        expect(result.state, fixture.expectedState);
        expect(result.connectionText, fixture.expectedConnection);
        expect(result.pastQuote, fixture.expectedPast);
        expect(result.currentQuote, fixture.expectedCurrent);
        expect(result.whatChangedText, fixture.expectedWhatChanged);

        // These fields feed the UI directly and must always remain usable even
        // when the model response is empty or incomplete.
        expect(result.connectionText.trim(), isNotEmpty);
        expect(result.whatChangedText.trim(), isNotEmpty);
        expect(result.pastQuote, isA<String>());
        expect(result.currentQuote, isA<String>());
      });
    }

    test('never throws for arbitrary malformed fragments', () {
      ...
      for (final fragment in fragments) {
        expect(() => parser.parse(fragment), returnsNormally);
      }
    });
  });
}

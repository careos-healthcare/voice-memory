...
void main() {
  ...
  group('CapacityBetaMissionEngine', () {
    ...
    test('computes first yes moment task', () {
      final none = engine.build(_input(capacityWedgeActive: true));
      expect(
    ...
    test('computes 3 yes moments task with return hint for 1-2 moments', () {
      ...
      final three = engine.build(
        _input(capacityWedgeActive: true, capacityMomentCount: 3),
      );
      expect(
  ...
  group('Routing and surfaces', () {
    ...
    test('beta signal dashboard links back to mission', () {
      final screen = File(
        'lib/screens/capacity_beta_signal_screen.dart',
      ).readAsStringSync();
    ...
    test('focused Archive home excludes the legacy beta mission card', () {
      final archive = File(
        'lib/screens/archive_belief_screen.dart',
      ).readAsStringSync();
      expect(archive, contains('ArchiveIntelligenceHome'));
      expect(archive, isNot(contains('archive_home_capacity_beta_mission')));
      expect(archive, isNot(contains('CapacityBetaMissionCard')));
    });

...
import 'package:voicememory_mobile/widgets/record/post_save_listening_card.dart';
...
void main() {
  ...
  group('PostSaveRecordedSummaryCard', () {
    testWidgets('shows saved confirmation before the transcript', (
      tester,
    ) async {
      await tester.pumpWidget(
        MaterialApp(
          ...
          home: Scaffold(
            body: PostSaveRecordedSummaryCard(
              entry: _entry(transcript: 'A real saved moment.'),
            ),
          ),
        ),
      );

      final savedTop = tester.getTopLeft(find.text('Saved.')).dy;
      final transcriptTop = tester
          .getTopLeft(find.text('A real saved moment.'))
          .dy;
      expect(savedTop, lessThan(transcriptTop));
    });
    ...
    testWidgets(
      'first entry orders transcript, observation, evidence, context, next',
      (tester) async {
        const transcript =
            'I feel uncertain about changing my career after another difficult week at work.';
        await tester.pumpWidget(
          MaterialApp(
            theme: AppTheme.light(),
            home: Scaffold(
              body: SingleChildScrollView(
                child: PostSaveRecordedSummaryCard(
                  entry: _entry(transcript: transcript),
                  allEntries: [_entry(transcript: transcript)],
                  primaryArchiveResult:
                      PostSavePrimaryArchiveKind.firstEntryFootnote,
                  onAddContext: () {},
                  onBackToRecord: () {},
                ),
              ),
        ...
        final transcriptFinder = find.byKey(
          const Key('post_save_recorded_summary_body'),
        );
        final observationFinder = find.byKey(
          const Key('post_save_first_observation'),
        );
        final evidenceFinder = find.byKey(
          const Key('post_save_first_observation_evidence'),
        );
        final contextFinder = find.byKey(const Key('post_save_add_context'));
        final nextFinder = find.byKey(const Key('post_save_record_another'));
        ...
        expect(
          ...
          lessThan(tester.getTopLeft(observationFinder).dy),
        );
        expect(
          tester.getTopLeft(observationFinder).dy,
          lessThan(tester.getTopLeft(evidenceFinder).dy),
        );
        expect(
          tester.getTopLeft(evidenceFinder).dy,
          lessThan(tester.getTopLeft(contextFinder).dy),
        );
        expect(
          tester.getTopLeft(contextFinder).dy,
          lessThan(tester.getTopLeft(nextFinder).dy),
        );
        expect(
          find.textContaining('This moment may be centered'),
          findsOneWidget,
        );
        expect(
          find.text(PostSaveRecordedSummaryCopy.firstEntryFootnote),
    ...
    testWidgets(
      'weak first entry shows low-evidence state without an observation',
      (tester) async {
        await tester.pumpWidget(
          MaterialApp(
            theme: AppTheme.light(),
            home: Scaffold(
              body: PostSaveRecordedSummaryCard(
                entry: _entry(transcript: 'Work was fine.'),
                allEntries: [_entry(transcript: 'Work was fine.')],
                primaryArchiveResult:
                    PostSavePrimaryArchiveKind.firstEntryFootnote,
              ),
            ),
        ...
        expect(
          find.byKey(const Key('post_save_first_observation_low_evidence')),
          findsOneWidget,
        );
        expect(
          find.byKey(const Key('post_save_first_observation')),
          findsNothing,
        );
        expect(
          find.byKey(const Key('post_save_first_observation_evidence')),
          findsNothing,
        );
      },
    );
        ...
        MaterialApp(
          ...
          home: Scaffold(
            body: SingleChildScrollView(
              child: Column(
                children: [
                  Text(BeliefProductCopy.reflectionSavedTitle),
